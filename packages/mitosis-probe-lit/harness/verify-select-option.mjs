...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/select-option.html');
await page.waitForFunction(() => customElements.get('lit-select-option'));
await page.waitForFunction(() =>
  document.querySelector('#default lit-select-option')?.shadowRoot?.querySelector('.option'),
);

const live = await page.evaluate(() => {
  const el = document.querySelector('#default lit-select-option');
...
const selected = await page.evaluate(() => {
  const el = document.querySelector('#selected lit-select-option');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-select-option');
...
const disabledParent = await page.evaluate(() => {
  const el = document.querySelector('#disabled-parent lit-select-option');
  return {
    hostOpacity: getComputedStyle(el).opacity,
    ariaDisabled: el.getAttribute('aria-disabled'),
  };
...
const highlighted = await page.evaluate(() => {
  const el = document.querySelector('#highlighted lit-select-option');
  const option = el.shadowRoot?.querySelector('.option');
...
const hidden = await page.evaluate(
  () => getComputedStyle(document.querySelector('#hidden lit-select-option')).display,
);

await browser.close();

const failures = [];
if (!live.isDefined) failures.push('lit-select-option is not defined');
...
if (live.checkIcon) failures.push('default shows check icon');
...
if (selected.iconName !== 'check') failures.push(`selected icon ${selected.iconName}`);
if (selected.iconTag !== 'P-ICON') failures.push(`selected icon tag ${selected.iconTag}`);
