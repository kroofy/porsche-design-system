...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // output/lit/src/Scroller.ts
  var LitScroller = class extends i4 {
    get cssText() {
      let prevVis = !!this._prevVisible;
      let nextVis = !!this._nextVisible;
      let hasBar = this.scrollbar ?? this.getAttribute("scrollbar");
      if (hasBar === true || hasBar === "true" || hasBar === "") {
        hasBar = true;
      } else {
        hasBar = false;
      }
      let isCompact = this.compact ?? this.getAttribute("compact");
      if (isCompact === true || isCompact === "true" || isCompact === "") {
        isCompact = true;
      } else {
        isCompact = false;
      }
      let isSticky = this.sticky ?? this.getAttribute("sticky");
    ...
    render() {
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style><span class="prev"></span><span class="next"></span><div class="scroll" tabindex="${this._prevVisible || this._nextVisible ? 0 : A}"><span class="sentinel"></span><slot></slot><span class="sentinel"></span></div></div>`;
    }
    firstUpdated() {
...
`;
  __decorateClass([
    n4()
  ], LitScroller.prototype, "scrollbar", 2);
  __decorateClass([
    n4()
  ], LitScroller.prototype, "compact", 2);
  __decorateClass([
    n4()
  ], LitScroller.prototype, "sticky", 2);
  LitScroller = __decorateClass([
    t3("lit-scroller")
  ], LitScroller);
})();
