...
useMetadata({ tagName: 'lit-button-pure' });
  ...
  const state = useStore({
    get cssText(): string {
      ...
      const color = disabled
        ...
        : colorMap[props.color || 'primary'] || colorMap.primary;
      ...
      const gapFor = (hide: any) => (hideFor(hide) ? '0' : 'var(--p-spacing-static-xs)');
      ...
      if (hasIcon) {
        out +=
          '.icon{position:relative;flex-shrink:0}.label{z-index:1;' +
          visFor(hideBase) +
          ';order:' +
          orderFor(alignBase) +
          '}';
        ...
        out +=
          '.label{position:relative;opacity:0}.icon{position:absolute;top:0;left:calc(50% - var(--p-leading-normal) / 2)}';
      } else {
      ...
      const keys: any = {};
      if (typeof size === 'object' && size !== null) for (const k of Object.keys(size)) keys[k] = 1;
      if (typeof stretch === 'object' && stretch !== null) for (const k of Object.keys(stretch)) keys[k] = 1;
      if (typeof hideLabel === 'object' && hideLabel !== null) for (const k of Object.keys(hideLabel)) keys[k] = 1;
      if (typeof alignLabel === 'object' && alignLabel !== null) for (const k of Object.keys(alignLabel)) keys[k] = 1;
