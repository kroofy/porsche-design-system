...
const PLAYGROUND_URL = 'http://localhost:3333/?components=button-pure';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_button_pure_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_button_pure_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/button_pure_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_button_pure_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-button-pure.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-button-pure'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="button-pure"] p-button-pure')];
  for (const el of hosts) {
    const lit = document.createElement('lit-button-pure');
...
await pauseCardAnimation(page);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="button-pure"] lit-button-pure')];
  return (
    hosts.length >= 19 &&
    hosts.every((el) => {
      const slot = el.shadowRoot?.querySelector('slot');
      if ((slot?.assignedNodes({ flatten: true }).length ?? 0) === 0) return false;
      const spinner = el.shadowRoot?.querySelector('p-spinner');
      const icon = el.shadowRoot?.querySelector('p-icon');
      if (spinner && getComputedStyle(spinner).display !== 'none') {
        return (
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="button-pure"] lit-button-pure')].filter((el) =>
      el.shadowRoot?.querySelector('.root'),
    ).length,
);
swap.fragment = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="button-pure"] lit-button-pure')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLitIcon = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="button-pure"] lit-button-pure')?.shadowRoot?.querySelector('lit-icon,lit-spinner'),
);
