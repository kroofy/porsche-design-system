...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var assignHide = (vars, bp, hidden) => {
    const p3 = bp === "base" ? "--p-rg-lw" : `--p-rg-lw-${bp}`;
    const d3 = bp === "base" ? "--p-rg-desc" : `--p-rg-desc-${bp}`;
    if (hidden) {
  ...
  var LitRadioGroup = class extends i4 {
    get hostStyle() {
      ...
      if (typeof direction === "object" && direction !== null) {
        ...
        for (const bp of BREAKPOINTS) {
          if (direction[bp] !== void 0) last = pick(direction, bp, "column");
    ...
    render() {
      return b2`<fieldset class="root" ?disabled=${!!this.isDisabled} role="radiogroup" aria-required=${this.isRequired ? "true" : A} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${this.hasLabel ? "label" : A} aria-describedby=${this.fieldsetDescribedBy()}>${this.labelNode}${this.descriptionNode}<div class="wrapper"><slot></slot>${this.spinnerNode}</div><span class="message" id="message" role=${this.messageRole}>${this.iconNode}${this.messageText}</span><span class="loading" id="loading" role="status">${this.loadingText}</span></fieldset>`;
    }
  };
      ...
      :host([hidden]) {
        ...
        @media (min-width: 760px) {
          .wrapper {
            flex-flow: var(
              --p-rg-flow-s,
              var(--p-rg-flow-xs, var(--p-rg-flow, column nowrap))
            );
            align-items: var(
              --p-rg-align-s,
              var(--p-rg-align-xs, var(--p-rg-align, stretch))
            );
          }
          .label-wrapper {
            min-width: var(
              --p-rg-lw-s-minw,
              var(--p-rg-lw-xs-minw, var(--p-rg-lw-minw))
            );
            position: var(
              --p-rg-lw-s-pos,
              var(--p-rg-lw-xs-pos, var(--p-rg-lw-pos, static))
            );
            ...
            overflow: var(
              --p-rg-lw-s-ov,
              var(--p-rg-lw-xs-ov, var(--p-rg-lw-ov, visible))
            );
            clip: var(
              --p-rg-lw-s-clip,
              var(--p-rg-lw-xs-clip, var(--p-rg-lw-clip, auto))
            );
            white-space: var(
              --p-rg-lw-s-ws,
              var(--p-rg-lw-xs-ws, var(--p-rg-lw-ws, normal))
            );
          }
          .label:is(span) {
            position: var(
              --p-rg-desc-s-pos,
              var(--p-rg-desc-xs-pos, var(--p-rg-desc-pos, static))
            );
            width: var(
              --p-rg-desc-s-w,
              var(--p-rg-desc-xs-w, var(--p-rg-desc-w, auto))
            );
            height: var(
              --p-rg-desc-s-h,
              var(--p-rg-desc-xs-h, var(--p-rg-desc-h, auto))
            );
            padding: var(
              --p-rg-desc-s-pad,
              var(--p-rg-desc-xs-pad, var(--p-rg-desc-pad, 0))
            );
            margin: var(--p-rg-desc-s-m, var(--p-rg-desc-xs-m, var(--p-rg-desc-m, 0)));
            overflow: var(
              --p-rg-desc-s-ov,
              var(--p-rg-desc-xs-ov, var(--p-rg-desc-ov, visible))
            );
            clip: var(
              --p-rg-desc-s-clip,
              var(--p-rg-desc-xs-clip, var(--p-rg-desc-clip, auto))
            );
            white-space: var(
              --p-rg-desc-s-ws,
              var(--p-rg-desc-xs-ws, var(--p-rg-desc-ws, normal))
            );
            margin-top: calc(-1 * var(--p-spacing-static-xs));
          }
        }
