...
const PLAYGROUND_URL = 'http://localhost:3333/?components=text';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_text_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_text_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/text_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_text_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-text.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-text'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="text"] p-text')];
  for (const el of hosts) {
    const lit = document.createElement('lit-text');
...
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="text"] lit-text')];
  return (
    hosts.length >= 12 &&
    hosts.every((el) => {
      const slot = el.shadowRoot?.querySelector('slot');
      return !!el.shadowRoot?.querySelector('p') && (slot?.assignedNodes({ flatten: true }).length ?? 0) > 0;
    })
  );
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="text"] lit-text')].filter((el) => el.shadowRoot?.querySelector('p'))
      .length
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="text"] lit-text')?.shadowRoot?.querySelector('my-fragment')
);
