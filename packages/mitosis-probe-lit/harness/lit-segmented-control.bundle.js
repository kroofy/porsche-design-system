...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // output/lit/src/SegmentedControl.ts
  var LitSegmentedControl = class extends i4 {
    ...
    syncItems() {
      ...
      for (const item of this.itemChildren()) {
        item.selected = String(item.value ?? item.getAttribute("value")) === String(value) && value !== null && value !== void 0;
        item.state = state2;
    ...
    render() {
      ...
      const labelBlock = hasLabel ? b2`<div class="label-wrapper"><div class="label" id="label" aria-disabled=${disabled ? "true" : A}>${this.labelText}<slot name="label"></slot></div><slot name="label-after"></slot></div>` : A;
      ...
      return b2`<fieldset class="root" ?disabled=${disabled} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${hasLabel ? "label" : A} aria-describedby=${hasDescription ? "description" : A}><style .innerHTML="${this.cssText}"></style>${labelBlock}${descBlock}<slot></slot><span class="message" id="message" role=${this.messageRole}>${icon}${this.messageText}</span></fieldset>`;
    }
  };
...
`;
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "form", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "name", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "value", 2);
  __decorateClass([
    n4()
