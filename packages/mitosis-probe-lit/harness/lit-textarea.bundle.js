...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitTextarea = class extends i4 {
    ...
    render() {
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style><div class="label-wrapper"><label class="label" id="label" for="textarea">${this.labelText}</label><slot name="label-after"></slot></div><span class="label" id="description">${this.descriptionText}</span><div class="wrapper"><textarea id="textarea" .value=${this.inputValue} placeholder=${this.placeholderText || A} name=${this.name || A} ?disabled=${!!this.isDisabled} ?readonly=${!!this.isReadOnly} maxlength=${this.maxLengthValue || A} rows=${this.rowsValue} aria-invalid=${this.ariaInvalid || A}></textarea><span class="sr-only" aria-live="polite">${this.srOnlyText}</span><span class="counter" aria-hidden="true">${this.counterText}</span></div><span class="message" id="message"><p-icon name=${this.iconName || A} color=${this.iconColor || A} aria-hidden="true"></p-icon>${this.messageText}</span></div>`;
    }
  };
