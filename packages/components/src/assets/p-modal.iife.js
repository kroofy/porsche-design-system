  ...
  var LitModal = class extends i4 {
    get hostStyle() {
      ...
      const writeBox = (bp, stretched) => {
        ...
        const s4 = bp === "base" ? "" : `-${bp}`;
        vars[`--p-mo-w${s4}`] = box.w;
        vars[`--p-mo-min-w${s4}`] = box.minW;
        vars[`--p-mo-max-w${s4}`] = box.maxW;
        vars[`--p-mo-place${s4}`] = box.place;
        vars[`--p-mo-margin${s4}`] = box.margin;
        vars[`--p-mo-radius${s4}`] = box.radius;
        vars[`--p-mo-clip${s4}`] = box.clip;
      };
    ...
    render() {
      ...
      return b2`<dialog ?inert=${!this.isOpenFlag} tabindex="-1" aria-modal="true" aria-label=${label}><div class="scroller"><div class="modal">${dismiss}<slot name="header"></slot><slot></slot><slot name="footer"></slot></div></div></dialog>`;
    }
  };
  LitModal.styles = i`
      :host {
        ...
        dialog {
          ...
          transition: visibility 0s linear var(--p-mo-delay),
            width 0s linear var(--p-mo-delay), height 0s linear var(--p-mo-delay),
            background-color var(--p-mo-ddur) var(--p-mo-ease),
            -webkit-backdrop-filter var(--p-mo-ddur) var(--p-mo-ease),
            backdrop-filter var(--p-mo-ddur) var(--p-mo-ease);
          overlay: none;
        }
        dialog:modal {
          overlay: auto;
        ...
        @supports (overlay: auto) and (transition-behavior: allow-discrete) {
          dialog {
            transition: visibility 0s linear var(--p-mo-delay),
              ...
              -webkit-backdrop-filter var(--p-mo-ddur) var(--p-mo-ease),
              backdrop-filter var(--p-mo-ddur) var(--p-mo-ease),
              overlay var(--p-mo-ddur) var(--p-mo-ease) allow-discrete;
          }
        }
        .scroller {
          position: absolute;
        ...
        .modal {
          ...
          transition: opacity var(--p-mo-pdur) var(--p-mo-ease),
            transform var(--p-mo-pdur) var(--p-mo-ease);
          width: var(--p-mo-w);
