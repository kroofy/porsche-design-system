...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitBanner = class extends i4 {
    get cssText() {
      ...
      let isOpen = this.open ?? this.getAttribute("open");
      if (isOpen === true || isOpen === "true" || isOpen === "") {
        isOpen = true;
      } else {
        isOpen = false;
      }
      let position = this.position ?? this.getAttribute("position");
    ...
    render() {
      ...
      let headingEl = A;
      if (heading) {
        if (tag === "h1") headingEl = b2`<h1>${heading}</h1>`;
        else if (tag === "h2") headingEl = b2`<h2>${heading}</h2>`;
        else if (tag === "h3") headingEl = b2`<h3>${heading}</h3>`;
      ...
      const desc = this.descriptionText ? b2`<p>${this.descriptionText}</p>` : hasDescriptionSlot ? b2`<slot name="description"></slot>` : b2`<slot></slot>`;
      const dismiss = this.showDismiss ? b2`<button class="dismiss" type="button" aria-description=${this.headingAria || A}><span>Close banner</span></button>` : A;
      return b2`<style .innerHTML="${this.cssText}"></style><div popover="manual" ?inert=${!this.isOpenFlag} role=${this.roleName} aria-live=${this.ariaLive} aria-label=${this.headingAria || A}><div class="notification">${headingEl}${desc}${dismiss}</div></div>`;
    }
  };
  LitBanner.styles = i`
      :host {
