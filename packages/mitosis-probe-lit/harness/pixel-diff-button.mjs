...
const PLAYGROUND_URL = 'http://localhost:3333/?components=button';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_button_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_button_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/button_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_button_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-button.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-button'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="button"] p-button')];
  for (const el of hosts) {
    const lit = document.createElement('lit-button');
    ...
    const compact = el.getAttribute('compact');
    if (compact !== null) {
      lit.setAttribute('compact', compact);
      lit.compact = compact;
    }
...
await pauseCardAnimation(page);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="button"] lit-button')];
  return (
    hosts.length >= 21 &&
    hosts.every((el) => {
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="button"] lit-button')].filter((el) =>
      el.shadowRoot?.querySelector('.root'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="button"] lit-button')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLitIcon = await page.evaluate(
  () => !!document.querySelector('[data-card="button"] lit-button')?.shadowRoot?.querySelector('lit-icon,lit-spinner'),
);
