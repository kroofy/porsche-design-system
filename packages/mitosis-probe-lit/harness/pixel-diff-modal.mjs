...
const BASELINE_PNG =
  process.env.BASELINE_PNG ?? '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_modal_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_modal_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/modal_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_modal_control.png`;
...
const hostReady = () => {
  const hosts = [...document.querySelectorAll('[data-card="modal"] p-modal, [data-card="modal"] lit-modal')];
  ...
  return (
    ...
    hosts.every((el) => {
      if (el.tagName === 'P-MODAL' && !el.classList.contains('hydrated')) return false;
...
await page.waitForSelector('[data-card="modal"] p-modal.hydrated', { timeout: 20_000, state: 'attached' });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-modal.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-modal');
...
swap.nested = await page.evaluate(() => {
  const modals = [...document.querySelectorAll('[data-card="modal"] lit-modal')];
  const buttons = [...document.querySelectorAll('[data-card="modal"] > p-button')];
  const texts = [...document.querySelectorAll('[data-card="modal"] lit-modal p-text')];
  return {
    modalCount: modals.length,
    buttonTags: [...new Set(buttons.map((n) => n.tagName))],
    textTags: [...new Set(texts.map((n) => n.tagName))],
    anyDialogOpen: modals.some((el) => el.shadowRoot?.querySelector('dialog')?.open),
  };
});
