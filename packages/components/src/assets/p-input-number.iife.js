...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/input-number/output/lit/InputNumber.ts
  var LitInputNumber = class extends i4 {
    get cssText() {
      const minWidth = {
        xs: 480,
        s: 760,
      ...
      const hasControls = isTrue(this.getAttribute("controls") ?? this.controls);
      ...
      let out = ":host{display:block;--_p-input-base-a:" + (compact ? "0.64285714" : "1") + ';--ref-p-input-slotted-padding:calc(11.2px * (var(--_p-input-base-a) - 0.64285714)) !important;--ref-p-input-slotted-margin:calc(-1 * calc(11.2px * (var(--_p-input-base-a) - 0.64285714))) !important}:host([hidden]){display:none !important}:host(:dir(rtl)) input::placeholder{direction:rtl;text-align:end}slot[name="label-after"]{display:inline-block;vertical-align:top}slot[name="label-after"]::slotted(*){margin-inline-start:var(--p-spacing-static-xs) !important}.label-after{display:inline-block;vertical-align:top}:not(:defined,[data-ssr]){visibility:hidden}input{all:unset;display:flex;flex:1;align-items:center;width:max(100%, 2ch);height:100%;font:var(--p-font-weight-normal) var(--p-typescale-sm) / calc(var(--p-leading-normal) + 6px) var(--p-font-porsche-next);text-overflow:ellipsis;-moz-appearance:textfield}input::-webkit-inner-spin-button,input::-webkit-outer-spin-button{display:none}.root{display:grid;gap:var(--p-spacing-static-xs)}.wrapper{display:flex;align-items:center;gap:calc(22.4px * (var(--_p-input-base-a) - 0.64285714) + 4px);height:calc(var(--_p-input-base-a) * 3.5rem);box-sizing:border-box;padding-inline:calc(22.4px * (var(--_p-input-base-a) - 0.64285714) + 8px);border:1px solid ' + palette.border + ";border-radius:" + (compact ? "var(--p-radius-lg)" : "var(--p-radius-xl)") + ";background:" + (readOnly ? "var(--p-color-frosted)" : palette.bg) + ";color:" + (readOnly ? "var(--p-color-contrast-medium)" : "var(--p-color-primary)") + ";cursor:" + (disabled ? "not-allowed" : "text") + ";transition:background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), border-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out)";
      if (readOnly) out += ";border-color:transparent";
      if (disabled) out += ";opacity:0.4";
      out += "}.wrapper:not(:has(input:disabled)):focus-within{border-color:" + palette.hover + "}";
      ...
      if (hasControls) {
        out += ".button{padding:var(--ref-p-input-slotted-padding);margin:var(--ref-p-input-slotted-margin)}";
      } else {
      ...
      if (disabled) {
        out += "@media(forced-colors:active){.wrapper{opacity:1;color:GrayText;border-color:GrayText}.wrapper>*{opacity:1;color:GrayText}.wrapper:not(:has(input:disabled)):focus-within{outline:2px solid Highlight;outline-offset:2px}.label{opacity:1;color:GrayText}}";
      } else {
        out += "@media(forced-colors:active){.wrapper:not(:has(input:disabled)):focus-within{outline:2px solid Highlight;outline-offset:2px}}";
      }
      if (!disabled && !readOnly && !loading) {
        out += "@media(hover:hover){.wrapper:hover:not(.button:hover), .label-wrapper:hover~.wrapper{border-color:" + palette.hover + "}}";
      }
      const keys = {};
    ...
    get isReadOnly() {
      return (this.readOnly ?? this.getAttribute("read-only")) === true || (this.readOnly ?? this.getAttribute("read-only")) === "true" || (this.readOnly ?? this.getAttribute("read-only")) === "";
    }
    get ariaDisabled() {
    ...
    get ariaReadonly() {
      return this.readOnly === true || this.readOnly === "true" || this.readOnly === "" ? "true" : "";
    }
    get loadingText() {
      const loading = this.loading === true || this.loading === "true" || this.loading === "";
    ...
    get minusIconSrc() {
      return "http://localhost:3001/icons/minus.f6d964c.svg";
    }
    get plusIconSrc() {
      return "http://localhost:3001/icons/plus.319993e.svg";
    }
    updated() {
    ...
    render() {
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style><div class="label-wrapper"><label class="label" id="label" for="input-number">${this.labelText}</label><slot name="label-after"></slot></div><span class="label" id="description">${this.descriptionText}</span><div class="wrapper"><slot name="start"></slot><input type="number" id="input-number" dir="auto" .value=${this.inputValue} placeholder=${this.placeholderText || A} name=${this.name || A} ?disabled=${!!this.isDisabled} ?readonly=${!!this.isReadOnly} maxlength=${this.maxLengthValue || A} aria-disabled=${this.ariaDisabled || A} aria-invalid=${this.ariaInvalid || A} aria-readonly=${this.ariaReadonly || A}><p-button-pure class="button" type="button" icon="minus" icon-source=${this.minusIconSrc || A} hide-label="true" tabindex="-1" ?disabled=${!!this.isDisabled || !!this.isReadOnly}>Decrement value by 1</p-button-pure><p-button-pure class="button" type="button" icon="plus" icon-source=${this.plusIconSrc || A} hide-label="true" tabindex="-1" ?disabled=${!!this.isDisabled || !!this.isReadOnly}>Increment value by 1</p-button-pure><slot name="end"></slot><p-spinner aria-hidden="true"></p-spinner></div><span class="message" id="message"><p-icon name=${this.iconName || A} source=${this.iconSrc || A} color=${this.iconColor || A} aria-hidden="true"></p-icon>${this.messageText}</span><span class="loading" id="loading" role="status">${this.loadingText}</span></div>`;
    }
  };
  LitInputNumber.styles = i`
      :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "disabled", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "loading", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "compact", 2);
  __decorateClass([
    n4({ attribute: "read-only" })
  ], LitInputNumber.prototype, "readOnly", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "controls", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "message", 2);
  __decorateClass([
    n4({ attribute: "hide-label" })
  ], LitInputNumber.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "label", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "name", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "description", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "value", 2);
  __decorateClass([
    n4({ attribute: "max-length" })
  ], LitInputNumber.prototype, "maxLength", 2);
  __decorateClass([
    n4()
  ], LitInputNumber.prototype, "placeholder", 2);
  LitInputNumber = __decorateClass([
    t3("p-input-number")
  ], LitInputNumber);
})();
