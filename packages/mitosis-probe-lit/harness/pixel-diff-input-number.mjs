...
const BASELINE_PNG =
  process.env.BASELINE_PNG ?? '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_input_number_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_input_number_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/input_number_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_input_number_control.png`;
...
const hostReady = () => {
  const hosts = [
    ...document.querySelectorAll('[data-card="input-number"] p-input-number, [data-card="input-number"] lit-input-number'),
  ];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('input[type="number"]')) return false;
...
await page.waitForSelector('[data-card="input-number"] p-input-number.hydrated', { timeout: 20_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-number.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-input-number');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-number"] lit-input-number')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="number"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-number"] lit-input-number')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="input-number"] lit-input-number')
