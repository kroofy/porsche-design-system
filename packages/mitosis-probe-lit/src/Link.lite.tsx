...
export default function LitLink(props: {
  ...
  const state = useStore({
    get cssText(): string {
      ...
      const gapFor = (h: any) =>
        isTrue(h) ? 'var(--p-link-gap,0)' : 'var(--p-link-gap,calc(11.2px * (var(--_p-link-a) - 0.64285714) + 4px))';
      ...
      const iconMarginFor = (h: any) =>
        isTrue(h) ? '0' : 'calc(-1 * (11.2px * (var(--_p-link-a) - 0.64285714) + 4px))';
      const bg =
        variant === 'secondary'
          ? 'var(--p-link-bg,var(--p-color-frosted-strong))'
          : 'var(--p-link-bg,var(--p-color-primary))';
      const fg =
        variant === 'secondary' ? 'var(--p-link-fg,var(--p-color-primary))' : 'var(--p-link-fg,var(--p-color-canvas))';
      const hoverBg =
        variant === 'secondary'
      ...
      for (const bp of Object.keys(keys)) {
        ...
        out +=
          ...
          '!important}';
        if (hasSlottedAnchor) {
          out += '::slotted(a)::before{border-radius:' + slottedRadiusFor(h, c) + '!important}';
        }
        out +=
          '.root{padding:' +
          padFor(h) +
