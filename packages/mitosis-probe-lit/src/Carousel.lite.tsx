...
useMetadata({ tagName: 'lit-carousel' });
  ...
  const state = useStore({
    get cssText(): string {
      ...
      const padXxl =
        ...
        col +
        ')';
      const pad = (v: any) => 'var(--p-carousel-ps,var(--p-carousel-px,' + v + '))';
      ...
      if (hasHeading || hasDescription) {
        out +=
          ...
          (isCenter ? ';text-align:center !important;justify-self:center !important' : '') +
          '}';
      }
      if (hasHeading) {
        out +=
      ...
      if (hasControls) {
        const just =
          alignControls !== 'auto' ? alignControls : isCenter ? 'center' : 'start';
        out +=
