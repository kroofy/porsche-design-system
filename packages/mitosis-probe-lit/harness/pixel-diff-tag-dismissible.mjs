...
const PLAYGROUND_URL = 'http://localhost:3333/?components=tag-dismissible';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_tag_dismissible_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_tag_dismissible_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/tag_dismissible_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_tag_dismissible_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-tag-dismissible.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-tag-dismissible'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="tag-dismissible"] p-tag-dismissible')];
  for (const el of hosts) {
    const lit = document.createElement('lit-tag-dismissible');
...
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="tag-dismissible"] lit-tag-dismissible')];
  return (
    hosts.length >= 2 &&
    hosts.every((el) => {
      const icon = el.shadowRoot?.querySelector('p-icon');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="tag-dismissible"] lit-tag-dismissible')].filter((el) =>
      el.shadowRoot?.querySelector('p-icon')
    ).length
);
swap.fragment = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="tag-dismissible"] lit-tag-dismissible')?.shadowRoot?.querySelector(
      'my-fragment'
    )
);
