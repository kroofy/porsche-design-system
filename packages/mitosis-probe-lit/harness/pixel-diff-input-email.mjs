...
const PLAYGROUND_URL = 'http://localhost:3333/?components=input-email';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_input_email_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_input_email_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/input_email_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_input_email_control.png`;
...
const hostReady = () => {
  const hosts = [...document.querySelectorAll('[data-card="input-email"] p-input-email, [data-card="input-email"] lit-input-email')];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('input[type="email"]')) return false;
...
await page.waitForSelector('[data-card="input-email"] p-input-email.hydrated', { timeout: 20_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-email.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-input-email'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="input-email"] p-input-email')];
  for (const el of hosts) {
    const lit = document.createElement('lit-input-email');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-email"] lit-input-email')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="email"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-email"] lit-input-email')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="input-email"] lit-input-email')?.shadowRoot?.querySelector('lit-icon,lit-spinner'),
);
