...
const PLAYGROUND_URL = 'http://localhost:3333/?components=checkbox';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_checkbox_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_checkbox_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/checkbox_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_checkbox_control.png`;
...
const hostReady = (el) => {
  if (!el.shadowRoot?.querySelector('input[type="checkbox"]')) return false;
...
await page.waitForSelector('[data-card="checkbox"] p-checkbox.hydrated', { timeout: 20_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-checkbox.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-checkbox'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="checkbox"] p-checkbox')];
  for (const el of hosts) {
    const lit = document.createElement('lit-checkbox');
...
await pauseCardAnimation(page);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="checkbox"] lit-checkbox')];
  return (
    hosts.length >= 13 &&
    hosts.every((el) => {
      if (!el.shadowRoot?.querySelector('input[type="checkbox"]')) return false;
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="checkbox"] lit-checkbox')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="checkbox"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="checkbox"] lit-checkbox')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="checkbox"] lit-checkbox')?.shadowRoot?.querySelector('lit-icon,lit-spinner'),
);
