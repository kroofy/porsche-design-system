...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // output/lit/src/InlineNotification.ts
  var LitInlineNotification = class extends i4 {
    ...
    render() {
      ...
      const action = this.actionLabelText ? b2`<p-button-pure class="action" icon=${this.actionIconName || A} ?loading=${this.actionLoadingFlag}>${this.actionLabelText}</p-button-pure>` : A;
      const dismiss = this.showDismiss ? b2`<button class="dismiss" type="button" aria-description=${this.headingAria || A}><span>Close notification</span></button>` : A;
      return b2`<style .innerHTML="${this.cssText}"></style><div class="notification" role=${this.roleName} aria-live=${this.ariaLive} aria-label=${this.headingAria || A}>${headingEl}${desc}${action}${dismiss}</div>`;
    }
  };
