...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitPagination = class extends i4 {
    get cssText() {
      ...
      let showLast = this.showLastPage ?? this.getAttribute("show-last-page") ?? this.getAttribute("showlastpage");
    ...
    createItems() {
      ...
      const range = (start, end) => Array.from(new Array(end - start + 1), (_2, i5) => i5 + start);
    ...
    render() {
      ...
      const lis = items.map((item, index) => {
        if (item.type === 2) {
          return b2`<li class="prev"><span role="button" tabindex=${item.isActive ? 0 : A} aria-label=${intl.prev} aria-disabled=${item.isActive ? A : "true"}><p-icon color="primary" aria-hidden="true" name="arrow-left"></p-icon></span></li>`;
        }
        if (item.type === 3) {
          return b2`<li class="next"><span role="button" tabindex=${item.isActive ? 0 : A} aria-label=${intl.next} aria-disabled=${item.isActive ? A : "true"}><p-icon color="primary" aria-hidden="true" name="arrow-right"></p-icon></span></li>`;
        }
        if (item.type === 1) {
          const side = index === 2 ? "ellip-start" : "ellip-end";
          return b2`<li class="ellip ${side}"><span class="ellipsis"></span></li>`;
        }
        const classes = [
        ...
        return b2`<li class=${classes || A}><span role="button" tabindex="0" aria-label=${intl.page + " " + item.value} aria-current=${item.isActive ? "page" : A}>${item.value}</span></li>`;
      });
      return b2`<nav aria-label=${intl.root}><style .innerHTML="${this.cssText}"></style><ul>${lis}</ul></nav>`;
    }
  };
...
`;
  __decorateClass([
    n4()
  ], LitPagination.prototype, "intl", 2);
  __decorateClass([
    n4()
  ], LitPagination.prototype, "totalItemsCount", 2);
  __decorateClass([
    n4()
  ], LitPagination.prototype, "itemsPerPage", 2);
  __decorateClass([
    n4()
  ], LitPagination.prototype, "activePage", 2);
  __decorateClass([
    n4()
  ], LitPagination.prototype, "showLastPage", 2);
  LitPagination = __decorateClass([
    t3("lit-pagination")
  ], LitPagination);
})();
