...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/text.html');
await page.waitForFunction(() => customElements.get('lit-text'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-text');
  return !!el?.shadowRoot?.querySelector('p') && el.textContent.includes('ABC');
});

const live = await page.evaluate(() => {
  const el = document.querySelector('#default lit-text');
  const sr = el.shadowRoot;
  const root = sr?.querySelector('p');
  ...
  return {
    isDefined: !!customElements.get('lit-text'),
    hasShadowRoot: !!sr,
    dynamicStyleTag: !!style,
    cssTextHasRoot: !!style?.textContent?.includes('p{'),
    cssTextHasTypescale: !!style?.textContent?.includes('var(--p-typescale-sm)'),
    cssTextHasSlotted: !!style?.textContent?.includes('::slotted(:is(p,span,div,address,blockquote,figcaption,cite,time,legend))'),
    cssTextHasMedia: !!document.querySelector('#bp lit-text')?.shadowRoot
      ?.querySelector('style')
      ?.textContent?.includes('@media(min-width:1000px)'),
    cssTextHasHyphensInherit: !!style?.textContent?.includes('hyphens:inherit'),
...
const xl = await page.evaluate(() => {
  const el = document.querySelector('#xl lit-text');
  const root = el.shadowRoot?.querySelector('p');
...
const inherit = await page.evaluate(() => {
  const el = document.querySelector('#inherit lit-text');
...
const info = await page.evaluate(() => {
  const el = document.querySelector('#info lit-text');
  const root = el.shadowRoot?.querySelector('p');
  const ref = document.querySelector('#info-ref');
  return {
    color: root ? getComputedStyle(root).color : null,
    colorRef: ref ? getComputedStyle(ref).color : null,
    match: root && ref ? getComputedStyle(root).color === getComputedStyle(ref).color : false,
  };
});

const bold = await page.evaluate(() => {
  const el = document.querySelector('#bold lit-text');
  const style = el.shadowRoot?.querySelector('style');
  const root = el.shadowRoot?.querySelector('p');
  return {
    cssHasBold: !!style?.textContent?.includes('font-weight-bold'),
    fontWeight: root ? getComputedStyle(root).fontWeight : null,
  };
});

const hidden = await page.evaluate(() => {
  const el = document.querySelector('#hidden lit-text');
  return getComputedStyle(el).display;
});

const rich = await page.evaluate(() => {
  const el = document.querySelector('#rich lit-text');
...
const bpGeometry = async (width) => {
  ...
  return page.evaluate(() => {
    const root = document.querySelector('#bp lit-text').shadowRoot.querySelector('p');
...
const failures = [];
if (!live.isDefined) failures.push('lit-text is not defined');
if (!live.hasShadowRoot) failures.push('no shadowRoot');
if (!live.dynamicStyleTag) failures.push('no cssText <style>');
if (!live.cssTextHasRoot) failures.push('cssText missing p rules');
if (!live.cssTextHasTypescale) failures.push('cssText missing typescale token');
if (!live.cssTextHasSlotted) failures.push('cssText missing ::slotted text reset');
if (!live.cssTextHasMedia) failures.push('breakpoint cssText missing 1000px media');
if (!live.cssTextHasHyphensInherit) failures.push('default hyphens is not inherit');
if (live.unknownWrapper) failures.push('my-fragment leaked');
if (live.rootTag !== 'P') failures.push(`root tag ${live.rootTag}`);
...
if (!info.match) failures.push(`info color ${info.color} != ref ${info.colorRef}`);
if (!bold.cssHasBold) failures.push('weight bold missing font-weight-bold');
