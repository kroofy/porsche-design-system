...
const PLAYGROUND_URL = 'http://localhost:3333/?components=crest';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_crest_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_crest_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/crest_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_crest_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-crest.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-crest');
...
await page.waitForFunction(() => {
  const img = document.querySelector('[data-card="crest"] lit-crest')?.shadowRoot?.querySelector('img');
  return !!img && img.complete && img.naturalWidth > 0;
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="crest"] lit-crest')].filter((el) =>
      el.shadowRoot?.querySelector('picture')
    ).length
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="crest"] lit-crest')?.shadowRoot?.querySelector('my-fragment')
);
