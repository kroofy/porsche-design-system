...
useMetadata({ tagName: 'lit-toast-item' });

export default function LitToastItem(props: { text?: string; state?: string }) {
  const state = useStore({
    get cssText(): string {
      ...
      return (
        ':host{display:block;max-width:inherit !important;box-sizing:border-box !important;' +
        'margin:0 !important;padding:0 !important;inset:inherit !important;border:0 !important;' +
        'outline:0 !important;overflow:visible !important;width:auto !important;height:auto !important;' +
        'background:transparent !important}' +
        ':host::backdrop{display:none}' +
        'slot:not([name]),slot[name="description"],p{all:unset;grid-area:1/2;margin-top:0px;' +
        'font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);' +
        'color:var(--p-color-primary)}' +
        '.notification{box-shadow:var(--p-shadow-lg);display:grid;' +
        'grid-template:repeat(3, auto) / auto minmax(0, 1fr) repeat(2, auto);' +
        'padding:calc(var(--p-spacing-static-sm) + var(--p-spacing-fluid-sm));' +
        'border-radius:var(--p-radius-2xl);background:' +
        bg +
        ...
        'font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);' +
        'background-color:var(--p-color-frosted-strong);color:var(--p-color-primary);cursor:pointer;' +
        '-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted);' +
        ...
        'grid-area:1/4/-1;align-self:flex-start;margin-block:calc(-6 * var(--p-spacing-static-2xs));' +
        ...
        '.dismiss span{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;' +
        'clip:rect(0,0,0,0);white-space:nowrap}' +
        '@media(min-width:760px){.notification::before{grid-area:1/1;place-self:flex-start;content:"";' +
        'width:1.5rem;height:1.5rem;margin-inline-end:var(--p-spacing-static-sm);background:' +
        color +
        ';-webkit-mask:' +
        iconMask +
        ';mask:' +
        ...
        '.dismiss{forced-color-adjust:none;background:Canvas;box-shadow:inset 0 0 0 2px ButtonBorder;color:ButtonText}' +
        '.dismiss:focus-visible{outline-color:Highlight}}' +
        '@media(hover:hover){.dismiss:hover{background-color:var(--p-color-frosted)}' +
