...
const BASELINE_PNG =
  ...
  '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_segmented_control_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_segmented_control_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/segmented_control_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_segmented_control_control.png`;
...
const hostReady = (tag) => {
  const hosts = [...document.querySelectorAll(`[data-card="segmented-control"] ${tag}`)];
  return (
    hosts.length >= 3 &&
    hosts.every((el) => {
      if (tag === 'p-segmented-control' && !el.classList.contains('hydrated')) return false;
...
await page.waitForFunction(hostReady, 'p-segmented-control', { timeout: 30_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-segmented-control.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-segmented-control');
    ...
    const noWrap = el.getAttribute('no-wrap');
    if (noWrap !== null) {
      lit.setAttribute('nowrap', noWrap);
      lit.noWrap = noWrap;
    }
    const items = [...el.querySelectorAll(':scope > p-segmented-control-item')];
...
await pauseCardAnimation(page);
await page.waitForFunction(hostReady, 'lit-segmented-control', { timeout: 30_000 });
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="segmented-control"] lit-segmented-control')].filter((el) =>
...
swap.fragment = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="segmented-control"] lit-segmented-control')
...
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="segmented-control"] lit-segmented-control')
      ?.shadowRoot?.querySelector('lit-icon,lit-segmented-control-item') ||
    !!document.querySelector('[data-card="segmented-control"] lit-segmented-control-item'),
);
swap.slottedCopied = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="segmented-control"] lit-segmented-control')].every(
      (el) => el.querySelectorAll(':scope > p-segmented-control-item').length >= 4,
    ),
);
swap.itemsStayStencil = await page.evaluate(() => {
  const items = [
    ...document.querySelectorAll('[data-card="segmented-control"] lit-segmented-control > p-segmented-control-item'),
  ];
  return {
    count: items.length,
    tags: [...new Set(items.map((n) => n.tagName))],
    icons: [
      ...new Set(
        items.flatMap((item) => [...(item.shadowRoot?.querySelectorAll('p-icon') ?? [])].map((n) => n.tagName)),
      ),
    ],
  };
});
...
const failed =
  ...
  !swap.slottedCopied ||
  swap.swapped < 3 ||
  swap.itemsStayStencil?.count < 12 ||
  swap.itemsStayStencil?.tags.join() !== 'P-SEGMENTED-CONTROL-ITEM' ||
