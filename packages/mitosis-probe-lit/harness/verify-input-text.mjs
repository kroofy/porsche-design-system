...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-text.html');
await page.waitForFunction(() => customElements.get('lit-input-text'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-text');
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-text'),
    hasShadowRoot: !!sr,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-text');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-text');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-text');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-text');
  const icon = el.shadowRoot?.querySelector('p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-text');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-text');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  return { overflow: wrap ? getComputedStyle(wrap).overflow : null };
});

const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-text')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-text');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-text is not defined');
...
if (live.inputType !== 'text') failures.push(`input type ${live.inputType}`);
