...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/heading.html');
await page.waitForFunction(() => customElements.get('lit-heading'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-heading');
  return !!el?.shadowRoot?.querySelector('h2') && el.textContent.includes('ABC');
});

const live = await page.evaluate(() => {
  const el = document.querySelector('#default lit-heading');
  const sr = el.shadowRoot;
  const root = sr?.querySelector('h2');
  ...
  return {
    isDefined: !!customElements.get('lit-heading'),
    hasShadowRoot: !!sr,
    dynamicStyleTag: !!style,
    cssTextHasRoot: !!style?.textContent?.includes('h2{'),
    ...
    cssTextHasMedia: !!document.querySelector('#bp lit-heading')?.shadowRoot
      ?.querySelector('style')
...
const xl = await page.evaluate(() => {
  const el = document.querySelector('#xl lit-heading');
  const root = el.shadowRoot?.querySelector('h2');
...
const inherit = await page.evaluate(() => {
  const el = document.querySelector('#inherit lit-heading');
...
const bold = await page.evaluate(() => {
  const el = document.querySelector('#bold lit-heading');
  const style = el.shadowRoot?.querySelector('style');
  const root = el.shadowRoot?.querySelector('h2');
  return {
...
const hidden = await page.evaluate(() => {
  const el = document.querySelector('#hidden lit-heading');
  return getComputedStyle(el).display;
});

const rich = await page.evaluate(() => {
  const el = document.querySelector('#rich lit-heading');
...
const bpGeometry = async (width) => {
  ...
  return page.evaluate(() => {
    const root = document.querySelector('#bp lit-heading').shadowRoot.querySelector('h2');
...
const failures = [];
if (!live.isDefined) failures.push('lit-heading is not defined');
if (!live.hasShadowRoot) failures.push('no shadowRoot');
if (!live.dynamicStyleTag) failures.push('no cssText <style>');
if (!live.cssTextHasRoot) failures.push('cssText missing h2 rules');
