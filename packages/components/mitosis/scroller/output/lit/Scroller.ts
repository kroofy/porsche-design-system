...
`;
  ...
  get cssText() {
    let prevVis: any = this.prevVis;
    let nextVis: any = this.nextVis;
  ...
  render() {
    return html`<div class="root"><style .innerHTML="${this.cssText}"></style><span class="prev" @click=${this.scrollPrev}></span><span class="next" @click=${this.scrollNext}></span><div class="scroll" tabindex=${this.prevVis || this.nextVis ? 0 : nothing}><span class="sentinel"></span><slot></slot><span class="sentinel"></span></div></div>`;
  }
}
