...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/banner/output/lit/Banner.ts
  var LitBanner = class extends i4 {
    constructor() {
      super(...arguments);
    ...
    get cssText() {
      ...
      let dismiss = this.getAttribute("dismiss-button") ?? this.dismissButton;
      if (dismiss === false || dismiss === "false") {
        dismiss = false;
      } else {
        dismiss = true;
      }
      let isOpen = this.getAttribute("open") ?? this.open;
      ...
      const maskMap = {
        ...
        success: `url('data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18m-1.26 12.69-3.8-3.8 1.07-1.05 2.73 2.73 5.25-5.26 1.06 1.06z"/></svg>')`,
        warning: `url('data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.58 18.26 13.3 3.75A1.5 1.5 0 0 0 12 3a1.5 1.5 0 0 0-1.3.75l-8.28 14.5a1.5 1.5 0 0 0 0 1.5c.28.47.76.75 1.3.75h16.56a1.5 1.5 0 0 0 1.3-2.25M13 17.5h-2v-2h2zm-.4-3.5h-1.2L11 8.5h2z"/></svg>')`,
        error: `url('data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18 3H6a2 2 0 0 0-2 2v12c0 1.1.9 2 2 2h4l2 2 2-2h4a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2m-5 3.5-.4 5.5h-1.2L11 6.5zm-2 7h2v2h-2z"/></svg>')`
      };
      ...
      const iconMask = (maskMap[visual] || maskMap.info) + " center/contain no-repeat";
      ...
      const topA = "translate3d(-50%,calc(-100% - var(--p-banner-top,var(--p-banner-position-top,56px))),0)";
      const topInset = "var(--p-banner-top,var(--p-banner-position-top,56px)) auto";
      const botA = "translate3d(-50%,calc(var(--p-banner-bottom,var(--p-banner-position-bottom,56px)) + 100%),0)";
      const botInset = "auto var(--p-banner-bottom,var(--p-banner-position-bottom,56px))";
      const posRule = (v2) => {
        if (v2 === "top")
      ...
      let out = ":host{display:contents}:host([hidden]){display:none !important}[popover]{all:unset;position:fixed;" + (typeof position === "object" && position !== null ? posRule(position.base || "bottom") : posRule(position)) + ";left:50vw;width:min(calc(100vw - 2 * var(--p-banner-inset-x,max(22px, 10.625vw - 12px))),var(--p-banner-max-w,100ch));transform:" + (isOpen ? "translate3d(-50%,0,0)" : "var(--_p-banner-a)") + ";transition:transform " + duration + " " + easing + ";overlay:none;display:none}[popover]:popover-open{overlay:auto;display:grid}[popover]::backdrop{display:none}";
      if (typeof position === "object" && position !== null) {
        for (const bp of Object.keys(position)) {
          if (bp === "base" || !minWidth[bp]) continue;
          out += "@media(min-width:" + minWidth[bp] + "px){[popover]{" + posRule(position[bp]) + "}}";
        }
      }
      out += "@supports (overlay: auto) and (transition-behavior: allow-discrete){[popover]{transition:transform " + duration + " " + easing + ",overlay " + duration + " " + easing + " allow-discrete, display " + duration + " " + easing + " allow-discrete}}";
      if (hasHeading) {
        out += 'slot[name="heading"],h1,h2,h3,h4,h5,h6{all:unset;grid-area:1/2;font:var(--p-font-weight-semibold) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);color:var(--p-color-primary)}';
      }
      out += 'slot:not([name]),slot[name="description"],p{all:unset;grid-area:' + descGrid + ";margin-top:" + descMargin + ";font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);color:var(--p-color-primary)}.notification{box-shadow:var(--p-shadow-lg);opacity:" + (isOpen ? "1" : "0") + ";transition:opacity " + duration + " " + easing + ";display:grid;grid-template:repeat(3, auto) / auto minmax(0, 1fr) repeat(2, auto);padding:calc(var(--p-spacing-static-sm) + var(--p-spacing-fluid-sm));border-radius:var(--p-radius-2xl);background:" + bg + ";-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted)}";
      if (dismiss) {
        out += '.dismiss{all:unset;box-sizing:border-box;display:grid;place-items:center;padding:6px;border-radius:var(--p-radius-full);font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);background-color:var(--p-color-frosted-strong);color:var(--p-color-primary);cursor:pointer;-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted);transition:background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out);grid-area:1/4/-1;align-self:flex-start;margin-block:calc(-6 * var(--p-spacing-static-2xs));margin-inline:var(--p-spacing-static-md) calc(-6 * var(--p-spacing-static-2xs))}.dismiss:focus-visible{outline:2px solid var(--p-color-focus);outline-offset:2px}.dismiss::before{content:"";width:var(--p-leading-normal);height:var(--p-leading-normal);-webkit-mask:' + closeMask + ";mask:" + closeMask + ";background:currentColor}.dismiss span{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}";
      }
      out += "@media(min-width:760px){.notification::before{grid-area:1/1;place-self:" + placeSelf + ';content:"";width:1.5rem;height:1.5rem;margin-inline-end:var(--p-spacing-static-sm);background:' + color + ";-webkit-mask:" + iconMask + ";mask:" + iconMask + "}@media(forced-colors:active){.notification::before{background:CanvasText}}}";
      out += "@media(forced-colors:active){.notification{outline:2px solid CanvasText;outline-offset:-2px}";
      if (dismiss) {
        out += ".dismiss{forced-color-adjust:none;background:Canvas;box-shadow:inset 0 0 0 2px ButtonBorder;color:ButtonText}.dismiss:focus-visible{outline-color:Highlight}";
      }
      out += "}";
      if (dismiss) {
        out += "@media(hover:hover){.dismiss:hover{background-color:var(--p-color-frosted)}@media(forced-colors:active){.dismiss:hover{background:Canvas}}}";
      }
    ...
    get headingNode() {
      ...
      if (heading) {
        ...
        if (tag === "h1") return b2`<h1>${heading}</h1>`;
        if (tag === "h2") return b2`<h2>${heading}</h2>`;
        if (tag === "h3") return b2`<h3>${heading}</h3>`;
        if (tag === "h4") return b2`<h4>${heading}</h4>`;
        if (tag === "h6") return b2`<h6>${heading}</h6>`;
        return b2`<h5>${heading}</h5>`;
      }
      return this.hasHeadingSlot ? b2`<slot name="heading"></slot>` : A;
    }
    get descriptionNode() {
      const description = this.descriptionText;
      if (description) return b2`<p>${description}</p>`;
      if (this.hasDescriptionSlot) return b2`<slot name="description"></slot>`;
      return b2`<slot></slot>`;
    }
    get dismissNode() {
      if (!this.showDismiss) return A;
      return b2`<button class="dismiss" type="button"><span>Close banner</span></button>`;
    }
    render() {
      return b2`<div popover="manual" inert=${this.isOpenFlag ? A : true} role=${this.roleName} aria-live=${this.ariaLive} aria-label=${this.headingAria || A}><style .innerHTML="${this.cssText}"></style><div class="notification">${this.headingNode}${this.descriptionNode}${this.dismissNode}</div></div>`;
    }
  };
  LitBanner.styles = i`
      :host {
...
`;
  __decorateClass([
    n4()
  ], LitBanner.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitBanner.prototype, "heading", 2);
  __decorateClass([
    n4({ attribute: "dismiss-button" })
  ], LitBanner.prototype, "dismissButton", 2);
  __decorateClass([
    n4()
  ], LitBanner.prototype, "open", 2);
  __decorateClass([
    n4()
  ], LitBanner.prototype, "position", 2);
  __decorateClass([
    n4({ attribute: "heading-tag" })
  ], LitBanner.prototype, "headingTag", 2);
  __decorateClass([
    n4()
  ], LitBanner.prototype, "description", 2);
  LitBanner = __decorateClass([
    t3("p-banner")
  ], LitBanner);
})();
