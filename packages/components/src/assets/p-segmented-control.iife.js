...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitSegmentedControl = class extends i4 {
    get cssText() {
      ...
      const colWidthFor = (col) => {
        if (col === "auto" || col === void 0 || col === null || col === "") {
          const w2 = measuredMax > maxCap && maxCap || measuredMax < minItem && measuredMin || measuredMax;
      ...
      let out = ':host([hidden]){display:none !important}slot[name="label-after"]{display:inline-block;vertical-align:top}slot[name="label-after"]::slotted(*){margin-inline-start:var(--p-spacing-static-xs) !important}.label-after{display:inline-block;vertical-align:top}:not(:defined,[data-ssr]){visibility:hidden}slot:not([name]){display:grid;grid-auto-rows:1fr';
      if (noWrap) {
      ...
      if (typeof columns === "object" && columns !== null)
        for (const k2 of Object.keys(columns)) keys[k2] = 1;
      for (const bp of Object.keys(keys)) {
        ...
        if (!noWrap && typeof columns === "object" && columns !== null && columns[bp] !== void 0) {
    ...
    render() {
      return b2`<fieldset class="root" ?disabled=${!!this.isDisabled} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${this.hasLabel ? "label" : A} aria-describedby=${this.hasDescription ? "description" : A}><style .innerHTML="${this.cssText}"></style>${this.labelNode}${this.descriptionNode}${this.slotNode}<span class="message" id="message" role=${this.messageRole}>${this.iconNode}${this.messageText}</span></fieldset>`;
    }
  };
