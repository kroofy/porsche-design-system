...
const BASELINE_PNG =
  ...
  '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_ai_tag_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_ai_tag_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/ai_tag_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_ai_tag_control.png`;
...
const hostReady = () => {
  const hosts = [
    ...document.querySelectorAll('[data-card="ai-tag"] p-ai-tag, [data-card="ai-tag"] lit-ai-tag'),
  ];
  return (
    hosts.length >= 3 &&
    hosts.every((el) => {
      if (!el.shadowRoot?.querySelector('div')) return false;
      if (el.tagName === 'P-AI-TAG' && !el.classList.contains('hydrated')) return false;
      return true;
    })
  );
};
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-ai-tag.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-ai-tag');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="ai-tag"] lit-ai-tag')].filter((el) =>
      el.shadowRoot?.querySelector('div'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="ai-tag"] lit-ai-tag')?.shadowRoot?.querySelector('my-fragment'),
);
swap.texts = await page.evaluate(() =>
  [...document.querySelectorAll('[data-card="ai-tag"] lit-ai-tag')].map((el) => {
