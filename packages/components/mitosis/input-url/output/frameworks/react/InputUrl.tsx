/* mitosis-native-host: native react from InputUrl.lite.tsx */
...
function LitInputUrl(props: LitInputUrlProps) {
  function cssText() {
    const minWidth: any = {
      xs: 480,
      s: 760,
    ...
    const hasIndicator = isTrue(props.indicator);
    const formState =
  ...
  return (
    <div
      className={["p-input-url", props.className].filter(Boolean).join(" ")}
      data-pds="input-url"
    >
      <div className="root">
        <style dangerouslySetInnerHTML={{ __html: scopeCss("\n        :host([hidden]) {\n          display: none !important;\n        }\n      " + cssText(), ".p-input-url") }} />
        <div className="label-wrapper">
          <label className="label" id="label" htmlFor="input-url">
            {labelText()}
          </label>
        ...
        <div className="wrapper">
          {props["start"] ?? null}
          <PIcon name="linked" color="contrast-low" aria-hidden="true" />
          <input type="url" id="input-url" dir="auto"  value={inputValue()} placeholder={placeholderText()} disabled={!!isDisabled()} readOnly={!!isReadOnly()} maxLength={maxLengthValue() || undefined} />
          {props["end"] ?? null}
