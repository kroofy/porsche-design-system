...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-week.html');
await page.waitForFunction(() => customElements.get('lit-input-week'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-week');
  return !!el?.shadowRoot?.querySelector('input[type="week"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-week'),
    hasShadowRoot: !!sr,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-week');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-week');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-week');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-week');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-week');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-week');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-week')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-week');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-week is not defined');
...
if (live.inputType !== 'week') failures.push(`input type ${live.inputType}`);
if (live.inputValue !== '2017-W01') failures.push(`input value ${live.inputValue}`);
