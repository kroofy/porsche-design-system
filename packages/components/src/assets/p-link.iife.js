...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/link/output/lit/Link.ts
  var LitLink = class extends i4 {
    get cssText() {
      ...
      if (hasIconCss) {
        out += ".icon,p-icon{font:var(--p-typescale-sm) var(--p-font-porsche-next);width:var(--p-leading-normal);height:var(--p-leading-normal);margin-inline-start:" + iconMarginFor(hideBase) + "}";
      }
      if (!hasVisibleIcon) {
      ...
      for (const bp of Object.keys(keys)) {
        ...
        if (hasIconCss) {
          out += ".icon,p-icon{margin-inline-start:" + iconMarginFor(h3) + "}";
        }
        out += "}";
      }
      return out;
    ...
    render() {
      return b2`
          ...
          <span class="root"><style .innerHTML="${this.cssText}"></style>
          <p-icon class="icon"
            size="inherit"
            color="inherit"
            aria-hidden="true"
