...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // output/lit/src/RadioGroupOption.ts
  var LitRadioGroupOption = class extends i4 {
    ...
    get isLoadingParent() {
      return (this.loadingParent ?? this.getAttribute("loading-parent") ?? this.getAttribute("loadingparent")) === true || (this.loadingParent ?? this.getAttribute("loading-parent") ?? this.getAttribute("loadingparent")) === "true" || (this.loadingParent ?? this.getAttribute("loading-parent") ?? this.getAttribute("loadingparent")) === "";
    }
    get isOptionLoading() {
      const selected = (this.selected ?? this.getAttribute("selected")) === true || (this.selected ?? this.getAttribute("selected")) === "true" || (this.selected ?? this.getAttribute("selected")) === "";
    ...
    render() {
      ...
      const spinner = optionLoading && !loadingParent ? b2`<p-spinner class="spinner" aria-hidden="true"></p-spinner>` : A;
      const loadingMsg = loadingParent ? A : b2`<span class="loading" id="loading" role="status">${this.loadingText}</span>`;
      const hasLabel = !!this.labelText || !!this.querySelector('[slot="label"]');
      const labelBlock = hasLabel ? b2`<div class="label-wrapper"><label class="label" id="label" for="radio-group-option" aria-disabled=${disabled || loading ? "true" : A}>${this.labelText}<slot name="label"></slot></label><span class="label-after"><slot name="label-after"></slot></span></div>` : A;
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style><div class="wrapper"><input id="radio-group-option" type="radio" name=${this.inputName || A} value=${this.inputValue} .checked=${selected} ?disabled=${disabled || loading} aria-invalid=${this.ariaInvalid || A} aria-disabled=${disabled || loading ? "true" : A} aria-describedby=${loading ? "loading" : A}>${spinner}</div>${labelBlock}${loadingMsg}</div>`;
    }
  };
