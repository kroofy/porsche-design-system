...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var assignBp = (vars, bp, size, stretch, hide, align) => {
    const s4 = bp === "base" ? "" : `-${bp}`;
    const hidden = isTrue(hide);
    const stretched = isTrue(stretch);
    vars[`--p-bp-host-d${s4}`] = stretched ? "block" : "inline-block";
    vars[`--p-bp-host-w${s4}`] = stretched ? "100%" : "auto";
    vars[`--p-bp-fs${s4}`] = fontFor(size);
    vars[`--p-bp-gap${s4}`] = hidden ? "0" : "var(--p-spacing-static-xs)";
    vars[`--p-bp-just${s4}`] = stretched ? "space-between" : "flex-start";
    vars[`--p-bp-ai${s4}`] = stretched ? "center" : "flex-start";
    vars[`--p-bp-inset${s4}`] = hidden ? "-2px" : "-4px";
    vars[`--p-bp-rad${s4}`] = hidden ? "var(--p-radius-full)" : "var(--p-radius-lg)";
    vars[`--p-bp-l-ws${s4}`] = hidden ? "nowrap" : "";
    vars[`--p-bp-l-ti${s4}`] = hidden ? "-999999px" : "0";
    vars[`--p-bp-l-ov${s4}`] = hidden ? "hidden" : "visible";
    vars[`--p-bp-l-ord${s4}`] = align === "start" || align === "left" ? "-1" : "0";
  };
  var LitButtonPure = class extends i4 {
    get hostStyle() {
      ...
      const vars = {
        "--p-bp-fg": disabled ? "var(--p-color-contrast-low)" : COLOR_MAP[this.color || "primary"] || COLOR_MAP.primary
      };
      const objecty = typeof size === "object" || typeof stretch === "object" || typeof hideLabel === "object" || typeof alignLabel === "object";
      if (objecty) {
        ...
        for (const bp of BREAKPOINTS) {
          ...
          if (stretchObj && stretchObj[bp] !== void 0)
            lastSt = pick(stretchObj, bp, lastSt);
          if (hideObj && hideObj[bp] !== void 0)
            lastH = pick(hideObj, bp, lastH);
          if (alignObj && alignObj[bp] !== void 0)
            lastA = pick(alignObj, bp, lastA);
          assignBp(vars, bp, lastS, lastSt, lastH, lastA);
        }
      } else {
        for (const bp of BREAKPOINTS)
          assignBp(vars, bp, size, stretch, hideLabel, alignLabel);
      }
      return vars;
    }
    get iconName() {
