...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var parse = (raw, fallback) => {
    ...
    if (typeof raw === "string" && raw.trim().charAt(0) === "{") {
      try {
        ...
        try {
          return JSON.parse(
            raw.replace(/'/g, '"').replace(/([{,]\s*)([A-Za-z_]\w*)\s*:/g, '$1"$2":')
          );
        } catch {
          return fallback;
        }
  ...
  var LitFlyout = class extends i4 {
    get hostStyle() {
      ...
      const fullscreen = parse(this.fullscreen ?? this.getAttribute("fullscreen"), false);
      ...
      const vars = {
        ...
        "--p-fo-sc-tf-rtl": isOpen ? "translate3d(0, 0, 0)" : isStart ? "translate3d(100%, 0, 0)" : "translate3d(-100%, 0, 0)",
      ...
      const writeBox = (bp, stretched) => {
        const box = boxFor(stretched, isStart);
        const s4 = bp === "base" ? "" : `-${bp}`;
        vars[`--p-fo-w${s4}`] = box.w;
        vars[`--p-fo-min-w${s4}`] = box.minW;
        vars[`--p-fo-max-w${s4}`] = box.maxW;
        vars[`--p-fo-clip${s4}`] = box.clip;
        vars[`--p-fo-clip-rtl${s4}`] = box.clipRtl;
        vars[`--p-fo-r-ss${s4}`] = box.rss;
        vars[`--p-fo-r-se${s4}`] = box.rse;
        vars[`--p-fo-r-es${s4}`] = box.res;
        vars[`--p-fo-r-ee${s4}`] = box.ree;
        vars[`--p-fo-hcm-is${s4}`] = box.hcmIs;
        vars[`--p-fo-hcm-ie${s4}`] = box.hcmIe;
      };
      if (typeof fullscreen === "object" && fullscreen !== null) {
        let last = pick(fullscreen, "base", false);
        for (const bp of BREAKPOINTS) {
          if (fullscreen[bp] !== void 0) last = pick(fullscreen, bp, last);
          writeBox(bp, last);
        }
      } else {
    ...
    render() {
      ...
      return b2`<dialog ?inert=${!this.isOpenFlag} tabindex="-1" aria-modal="true" aria-label=${label}><div class="scroller"><div class="flyout">${dismiss}<slot name="header"></slot><slot></slot><slot name="footer"></slot><slot name="sub-footer"></slot></div></div></dialog>`;
    }
  };
  LitFlyout.styles = i`
      :host {
          ...
          --ref-p-flyout-pb: calc(
            var(--p-radius-3xl) + var(--p-spacing-fluid-md)
          ) !important;
          --ref-p-flyout-px: var(--p-spacing-fluid-lg) !important;
          --pds-internal-grid-outer-column: calc(
            var(--p-spacing-fluid-lg) - clamp(16px, 1.25vw + 12px, 36px)
          ) !important;
        ...
        slot[name="header"] {
          ...
          position: sticky;
          top: 0;
          margin-block: calc(-1 * var(--p-spacing-fluid-md))
            calc(-1 * var(--p-spacing-static-md));
          padding: var(--p-spacing-fluid-md) var(--p-spacing-fluid-lg)
        ...
        slot[name="footer"] {
          ...
          bottom: -0.1px;
          margin-block: calc(
            -1 * calc(calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) -
                  var(--p-radius-3xl))
          );
          padding: calc(
              calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) -
                var(--p-radius-3xl)
            )
            var(--p-spacing-fluid-lg);
          background: linear-gradient(
            0deg,
            var(--_p-dialog-a) 0%,
            var(--_p-dialog-a) 20%,
        ...
        slot[name="footer"][data-stuck]::after {
          ...
          inset: calc(
              calc(
                  calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) -
                    var(--p-radius-3xl)
                ) - 12 * var(--p-spacing-static-2xs)
            )
            calc(var(--p-spacing-fluid-lg) - 12 * var(--p-spacing-static-2xs));
        ...
        dialog {
          ...
          transition: visibility 0s linear var(--p-fo-delay),
            width 0s linear var(--p-fo-delay), height 0s linear var(--p-fo-delay),
            background-color var(--p-fo-ddur) var(--p-fo-ease),
            -webkit-backdrop-filter var(--p-fo-ddur) var(--p-fo-ease),
            backdrop-filter var(--p-fo-ddur) var(--p-fo-ease);
          overlay: none;
        }
        dialog:modal {
          overlay: auto;
        ...
        @supports (overlay: auto) and (transition-behavior: allow-discrete) {
          dialog {
            transition: visibility 0s linear var(--p-fo-delay),
              ...
              -webkit-backdrop-filter var(--p-fo-ddur) var(--p-fo-ease),
              backdrop-filter var(--p-fo-ddur) var(--p-fo-ease),
              overlay var(--p-fo-ddur) var(--p-fo-ease) allow-discrete;
          }
        }
        .scroller {
          position: absolute;
          ...
          transition: opacity var(--p-fo-pdur) var(--p-fo-ease),
            transform var(--p-fo-pdur) var(--p-fo-ease);
        }
        .scroller:dir(rtl) {
        ...
        .flyout {
          position: relative;
          display: grid;
          grid-template: auto / var(--p-spacing-fluid-sm) minmax(0, 1fr) var(
              --p-spacing-fluid-sm
            );
          grid-template-rows: var(--p-fo-rows, auto);
          gap: var(--p-spacing-fluid-md)
            calc(var(--p-spacing-fluid-lg) - var(--p-spacing-fluid-sm));
