...
const BASELINE_PNG =
  process.env.BASELINE_PNG ?? '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_textarea_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_textarea_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/textarea_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_textarea_control.png`;
...
const hostReady = () => {
  const hosts = [
    ...document.querySelectorAll('[data-card="textarea"] p-textarea, [data-card="textarea"] lit-textarea'),
  ];
  return (
    hosts.length >= 5 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('textarea')) return false;
...
await page.waitForFunction(() => customElements.get('p-textarea') && customElements.get('p-icon'), {
  timeout: 20_000,
});
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-textarea.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-textarea');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="textarea"] lit-textarea')].filter((el) =>
      el.shadowRoot?.querySelector('textarea'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="textarea"] lit-textarea')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () => !!document.querySelector('[data-card="textarea"] lit-textarea')?.shadowRoot?.querySelector('lit-icon'),
);
