...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/textarea/output/lit/Textarea.ts
  var LitTextarea = class extends i4 {
    get cssText() {
      const minWidth = {
        xs: 480,
        s: 760,
      ...
      let out = ":host{display:block;--_p-textarea-a:" + (compact ? "0.64285714" : "1") + '}:host([hidden]){display:none !important}slot[name="label-after"]{display:inline-block;vertical-align:top}slot[name="label-after"]::slotted(*){margin-inline-start:var(--p-spacing-static-xs) !important}.label-after{display:inline-block;vertical-align:top}:not(:defined,[data-ssr]){visibility:hidden}textarea{all:unset;grid-area:1/1;display:block;resize:' + resize + ";field-sizing:var(--p-textarea-field-sizing,unset);min-width:var(--p-textarea-min-width,2ch);max-width:var(--p-textarea-max-width,unset);min-height:var(--p-textarea-min-height,calc(var(--_p-textarea-a) * 3.5rem));max-height:var(--p-textarea-max-height,unset);border:1px solid " + (readOnly ? "transparent" : palette.border) + ";border-radius:" + (compact ? "var(--p-radius-lg)" : "var(--p-radius-xl)") + ";background:" + (readOnly ? "var(--p-color-frosted)" : palette.bg) + ";color:" + (readOnly ? "var(--p-color-contrast-medium)" : "var(--p-color-primary)") + ";box-sizing:border-box;transition:background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), border-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out);font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);padding:" + padding + ";cursor:" + (disabled ? "not-allowed" : "text") + "}textarea:focus{border-color:" + palette.hover + "}";
      if (!disabled && !readOnly) {
        out += "@media(hover:hover){textarea:hover, .label-wrapper:hover~textarea{border-color:" + palette.hover + "}}";
      }
      out += ".root{display:grid;gap:var(--p-spacing-static-xs)}.wrapper{display:grid";
      if (disabled) out += ";opacity:0.4";
      out += "}";
      if (hasCounter) {
        out += ".counter{pointer-events:none;max-width:100%;box-sizing:border-box;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);color:var(--p-color-contrast-high);grid-area:1/1;place-self:flex-end;margin-inline-end:" + padInline + ";margin-bottom:calc(11.2px * (var(--_p-textarea-a) - 0.64285714) + 4px)}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}";
      } else {
    ...
    get counterText() {
      ...
      const max = (this.maxLength ?? this.getAttribute("max-length") ?? this.getAttribute("maxlength")) == null || (this.maxLength ?? this.getAttribute("max-length") ?? this.getAttribute("maxlength")) === "" ? "" : String(this.maxLength ?? this.getAttribute("max-length") ?? this.getAttribute("maxlength"));
    ...
    get srOnlyText() {
      ...
      const max = (this.maxLength ?? this.getAttribute("max-length") ?? this.getAttribute("maxlength")) == null || (this.maxLength ?? this.getAttribute("max-length") ?? this.getAttribute("maxlength")) === "" ? "" : String(this.maxLength ?? this.getAttribute("max-length") ?? this.getAttribute("maxlength"));
      if (max)
        return "You have " + (Number(max) - value.length) + " out of " + max + " characters left";
    ...
    render() {
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style><div class="label-wrapper"><label class="label" id="label" for="textarea">${this.labelText}</label><slot name="label-after"></slot></div><span class="label" id="description">${this.descriptionText}</span><div class="wrapper"><textarea id="textarea" .value=${this.inputValue} placeholder=${this.placeholderText || A} name=${this.name || A} rows=${this.rowsValue || A} maxlength=${this.maxLengthValue || A} ?disabled=${!!this.isDisabled} ?readonly=${!!this.isReadOnly} aria-invalid=${this.ariaInvalid || A}></textarea><span class="sr-only" aria-live="polite">${this.srOnlyText}</span><span class="counter" aria-hidden="true">${this.counterText}</span></div><span class="message" id="message"><p-icon name=${this.iconName || A} source=${this.iconSrc || A} color=${this.iconColor || A} aria-hidden="true"></p-icon>${this.messageText}</span></div>`;
    }
  };
  LitTextarea.styles = i`
      :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "disabled", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "compact", 2);
  __decorateClass([
    n4({ attribute: "read-only" })
  ], LitTextarea.prototype, "readOnly", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "counter", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "resize", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "message", 2);
  __decorateClass([
    n4({ attribute: "hide-label" })
  ], LitTextarea.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "label", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "name", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "description", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "value", 2);
  __decorateClass([
    n4({ attribute: "max-length" })
  ], LitTextarea.prototype, "maxLength", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "rows", 2);
  __decorateClass([
    n4()
  ], LitTextarea.prototype, "placeholder", 2);
  LitTextarea = __decorateClass([
    t3("p-textarea")
  ], LitTextarea);
})();
