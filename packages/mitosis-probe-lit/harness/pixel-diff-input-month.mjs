...
const hostReady = () => {
  const hosts = [
    ...document.querySelectorAll('[data-card="input-month"] p-input-month, [data-card="input-month"] lit-input-month'),
  ];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('input[type="month"]')) return false;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-month.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-input-month');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-month"] lit-input-month')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="month"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-month"] lit-input-month')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="input-month"] lit-input-month')
