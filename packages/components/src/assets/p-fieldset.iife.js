...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/fieldset/output/lit/Fieldset.ts
  var LitFieldset = class extends i4 {
    get cssText() {
      ...
      let out = ":host{display:block}:host([hidden]){display:none !important}:not(:defined,[data-ssr]){visibility:hidden}fieldset{all:unset;display:block}";
      if (hasLabel) {
        out += "legend{all:unset;margin-bottom:var(--p-spacing-static-md);color:var(--p-color-primary);font:" + (small ? "var(--p-font-weight-semibold) var(--p-typescale-sm)" : "var(--p-font-weight-normal) var(--p-typescale-md)") + " / var(--p-leading-normal) var(--p-font-porsche-next)}";
      } else {
        out += "legend{display:none}";
      }
      out += ".required{user-select:none}.message{display:flex;gap:var(--p-spacing-static-xs);font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next)";
      if (messageColor) out += ";color:" + messageColor;
      out += ";transition:color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), opacity var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out);margin-top:var(--p-spacing-static-md)}.message:empty{opacity:0;position:absolute}";
      if (!hasMsg)
    ...
    render() {
      return b2`<fieldset><style .innerHTML="${this.cssText}"></style><legend>${this.labelText}</legend><slot></slot><span class="message" id="message"><p-icon name=${this.iconName || A} source=${this.iconSrc || A} color=${this.iconColor || A} aria-hidden="true"></p-icon>${this.messageText}</span></fieldset>`;
    }
  };
  LitFieldset.styles = i`
      :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitFieldset.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitFieldset.prototype, "message", 2);
  __decorateClass([
    n4()
  ], LitFieldset.prototype, "label", 2);
  __decorateClass([
    n4({ attribute: "label-size" })
  ], LitFieldset.prototype, "labelSize", 2);
  LitFieldset = __decorateClass([
    t3("p-fieldset")
  ], LitFieldset);
})();
