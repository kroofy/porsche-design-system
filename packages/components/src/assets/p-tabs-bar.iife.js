  ...
  var LitTabsBar = class extends i4 {
    constructor() {
      ...
      this.onTabClick = (e5) => {
        ...
        const newIndex = tabs.findIndex((el) => el.contains(e5.target));
        if (newIndex >= 0) this.emitUpdate(newIndex);
      };
      this.onTabKeydown = (e5) => {
        let upcoming;
        const root = this.getRootNode();
        const hasPTabsParent = root?.host?.tagName === "P-TABS";
        const tabs = this.tabChildren();
        const activeElementIndex = hasPTabsParent ? this.sanitizedIndex() ?? 0 : Math.max(tabs.indexOf(document.activeElement), 0);
        ...
        switch (e5.key) {
          case "ArrowLeft":
          case "Left":
            upcoming = tabs.length ? (activeElementIndex + (isRtl ? 1 : -1) + tabs.length) % tabs.length : void 0;
            break;
          case "ArrowRight":
          case "Right":
            upcoming = tabs.length ? (activeElementIndex + (isRtl ? -1 : 1) + tabs.length) % tabs.length : void 0;
    ...
    sanitizedIndex() {
      ...
      if (!Number.isInteger(n5) || count < 1 || n5 < 0 || n5 > count - 1) return void 0;
      return n5;
    }
    scrollerAria() {
      if (!this.isTabList()) return A;
    ...
    syncScrollerAria() {
      ...
      for (const [key, value] of Object.entries(aria)) {
        if (key === "role" || value === void 0 || value === null) continue;
    ...
    syncTabAria() {
      ...
      tabs.forEach((tab, index) => {
        ...
        if (list) {
          ...
          tab.setAttribute("tabindex", active !== void 0 ? isActive ? "0" : "-1" : index === 0 ? "0" : "-1");
...
`;
  ...
  __decorateClass([
    n4()
  ], LitTabsBar.prototype, "compact", 2);
  __decorateClass([
    n4()
  ], LitTabsBar.prototype, "background", 2);
  __decorateClass([
    n4()
