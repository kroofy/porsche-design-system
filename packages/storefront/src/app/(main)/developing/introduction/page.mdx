...
<TableOfContents headings={['Start Coding', 'Native elements', 'Requirements']} />
...
If the control is one native HTML tag, PDS does not wrap it. Use the parallel [`./elements`](/developing/elements)
export for a real `button`, `input`, `dialog`, and the rest. The public `.` import still returns generated
custom-element wrappers. Stencil custom-element hosts stay until a breaking major.
