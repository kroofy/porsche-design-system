...
useMetadata({ tagName: 'p-link-tile' });
...
const assignBp = (vars: Record<string, string>, bp: string, size: any, weight: any, ratio: any, compact: any) => {
  ...
  vars[`--p-lt-fs${s}`] = fontFor(size);
  vars[`--p-lt-fw${s}`] = weightFor(weight);
  vars[`--p-lt-ar${s}`] = String(ratio || '4/3');
  vars[`--p-lt-foot-d${s}`] = on ? 'grid' : 'flex';
  vars[`--p-lt-foot-cols${s}`] = on ? 'minmax(0, 1fr) auto' : 'none';
  vars[`--p-lt-foot-gap${s}`] = on ? 'var(--p-spacing-static-md)' : '0';
  vars[`--p-lt-foot-dir${s}`] = on ? 'row' : 'column';
  vars[`--p-lt-foot-ai${s}`] = on ? 'stretch' : 'start';
  vars[`--p-lt-pure-d${s}`] = on ? 'inline-block' : 'none';
  vars[`--p-lt-btn-d${s}`] = on ? 'none' : 'inline-block';
};

export default function LitLinkTile(props: {
  size?: any;
  weight?: any;
  aspectRatio?: any;
  label?: string;
