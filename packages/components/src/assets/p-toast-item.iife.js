...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitToastItem = class extends i4 {
    get hostStyle() {
      ...
      return {
        "--p-toast-item-bg": BG[visual] || BG.info,
        "--p-toast-item-color": COLOR[visual] || COLOR.info,
        "--p-toast-item-icon": ICON[visual] || ICON.info
      };
    }
    get textValue() {
    ...
    render() {
      return b2`<div class="notification"><p>${this.textValue}</p><button class="dismiss" type="button"><span>Close notification message</span></button></div>`;
    }
  };
  LitToastItem.styles = i`
      :host {
        ...
        .notification {
          ...
          background: var(--p-toast-item-bg);
        ...
        @media (min-width: 760px) {
          .notification::before {
            ...
            background: var(--p-toast-item-color);
            -webkit-mask: var(--p-toast-item-icon);
            mask: var(--p-toast-item-icon);
          }
