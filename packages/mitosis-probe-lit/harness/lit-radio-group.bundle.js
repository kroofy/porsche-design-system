...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitRadioGroup = class extends i4 {
    ...
    render() {
      ...
      const labelBlock = hasLabel ? b2`<div class="label-wrapper"><div class="label" id="label" aria-disabled=${labelDisabled ? "true" : A}>${this.labelText}<slot name="label"></slot></div><slot name="label-after"></slot></div>` : A;
      const descBlock = hasDescription ? b2`<span class="label" id="description" aria-disabled=${labelDisabled ? "true" : A}>${this.descriptionText}<slot name="description"></slot></span>` : A;
      const spinner = loading ? b2`<p-spinner class="spinner" aria-hidden="true"></p-spinner>` : A;
      const icon = this.iconName ? b2`<p-icon name=${this.iconName} color=${this.iconColor || A} aria-hidden="true"></p-icon>` : A;
      return b2`<fieldset class="root" ?disabled=${disabled} role="radiogroup" aria-required=${required ? "true" : A} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${hasLabel ? "label" : A} aria-describedby=${this.fieldsetDescribedBy()}><style .innerHTML="${this.cssText}"></style>${labelBlock}${descBlock}<div class="wrapper"><slot></slot>${spinner}</div><span class="message" id="message" role=${this.messageRole}>${icon}${this.messageText}</span><span class="loading" id="loading" role="status">${this.loadingText}</span></fieldset>`;
    }
  };
