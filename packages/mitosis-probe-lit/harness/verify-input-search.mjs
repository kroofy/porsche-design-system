...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-search.html');
await page.waitForFunction(() => customElements.get('lit-input-search'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-search');
  return !!el?.shadowRoot?.querySelector('input[type="search"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-search'),
    hasShadowRoot: !!sr,
    ...
    cssTextHasSearchCancel: !!style?.textContent?.includes('::-webkit-search-cancel-button'),
    ...
    clearTag: clear?.tagName ?? null,
    clearIsLit: clear?.tagName === 'LIT-BUTTON-PURE',
    clearIcon: clear?.getAttribute('icon'),
    clearClass: clear?.getAttribute('class'),
    clearHidden: clear ? getComputedStyle(clear).display === 'none' : true,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-search');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-search');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-search');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-search');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-search');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-search');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-search')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-search');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-search is not defined');
...
if (!live.cssTextHasSearchCancel) failures.push('cssText missing webkit search cancel');
...
if (live.inputType !== 'search') failures.push(`input type ${live.inputType}`);
...
if (live.indicatorName !== 'search') failures.push(`indicator name ${live.indicatorName}`);
if (live.indicatorColor !== 'contrast-medium') failures.push(`indicator color ${live.indicatorColor}`);
...
if (live.clearTag !== 'P-BUTTON-PURE') failures.push(`clear tag ${live.clearTag}`);
if (live.clearIsLit) failures.push('clear was swapped to lit-button-pure');
if (live.clearIcon !== 'close') failures.push(`clear icon ${live.clearIcon}`);
if (live.clearClass !== 'button') failures.push(`clear class ${live.clearClass}`);
if (live.clearHidden) failures.push('default clear hidden');
if (live.clearText !== 'Clear field') failures.push(`clear text ${live.clearText}`);
...
if (!hideLabel.clearHidden) failures.push('hide-label host still shows clear');
