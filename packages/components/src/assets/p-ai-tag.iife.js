...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitAiTag = class extends i4 {
    get cssText() {
      const iconMask = `url('data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M10.85 6.39c.54 2.84.8 4.26 1.65 5.1s2.27 1.12 5.11 1.66c.52.18.52.52 0 .7-2.82.53-4.24.8-5.09 1.63-.86.85-1.13 2.27-1.67 5.13-.18.52-.52.52-.7 0-.54-2.84-.8-4.26-1.65-5.1s-2.27-1.12-5.11-1.66c-.52-.18-.52-.52 0-.7 2.84-.54 4.26-.8 5.1-1.65s1.12-2.27 1.66-5.11c.18-.52.52-.52.7 0m6.81-3.2c.25 1.32.38 1.98.77 2.38s1.06.52 2.39.77c.24.08.24.24 0 .32-1.3.25-1.97.38-2.36.75-.41.4-.54 1.06-.8 2.4-.08.25-.24.25-.32 0-.24-1.25-.37-1.91-.72-2.31-.39-.45-1.05-.57-2.44-.84-.24-.08-.24-.24 0-.32 1.33-.25 1.99-.38 2.38-.77s.53-1.06.78-2.39c.08-.24.24-.24.32 0"/></svg>') center/contain no-repeat`;
      return ':host{display:inline-flex;vertical-align:top;white-space:nowrap;font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next) !important}:host([hidden]){display:none !important}abbr{all:unset}div{display:flex;align-items:center;gap:2px;padding:0 var(--p-spacing-static-sm) 0 var(--p-spacing-static-xs);border-radius:calc(var(--p-spacing-static-xs) + (var(--p-leading-normal) / 2));font-size:var(--p-typescale-2xs);color:var(--p-color-contrast-high);background:var(--p-color-frosted-strong);-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted);transition:color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), backdrop-filter var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out)}div::before{content:"";width:1rem;height:1rem;background:var(--p-color-contrast-high);mask:' + iconMask + ";-webkit-mask:" + iconMask + "}@media(forced-colors:active){div{outline:1px solid transparent}div::before{background:CanvasText}}";
    }
    get translation() {
    ...
    get shortLabel() {
      return this.translation.short;
    }
    get longLabel() {
      return this.translation.long;
    }
    get copyLabel() {
      const variant = (this.getAttribute("variant") ?? this.variant) || "generated";
      if (variant === "modified") return this.translation.modified;
      return this.translation.generated;
    }
    render() {
      return this.isAbbreviation ? b2`<div><style .innerHTML="${this.cssText}"></style><abbr title=${this.longLabel}>${this.shortLabel}</abbr></div>` : b2`<div><style .innerHTML="${this.cssText}"></style>${this.copyLabel}</div>`;
    }
  };
  LitAiTag.styles = i`
      :host {
...
`;
  __decorateClass([
    n4()
  ], LitAiTag.prototype, "locale", 2);
  __decorateClass([
    n4()
  ], LitAiTag.prototype, "variant", 2);
  LitAiTag = __decorateClass([
    t3("p-ai-tag")
  ], LitAiTag);
})();
