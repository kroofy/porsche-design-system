...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitPagination = class extends i4 {
    ...
    get pageItems() {
      ...
      const createRange = (start, end) => Array.from(new Array(end - start + 1), (_2, i5) => i5 + start);
      const items = [
      ...
      if (1 + 2 + 2 >= pageTotal) {
        ...
        const showPageInsteadOfLastEllipsis = lastEllipsisPageNumber === pageTotal - (showLast ? 1 : 0);
    ...
    get pageNodes() {
      return this.pageItems.map((item, index) => {
        ...
        if (item.type === 1) {
          ...
          return b2`<li class="ellip ellip-${which}"><span class="ellipsis"></span></li>`;
        }
        const cls = [
        ...
        return b2`<li class=${cls}><span role="button" tabindex="0" aria-label=${"Page " + item.value} aria-current=${item.isActive ? "page" : A}>${item.value}</span></li>`;
      });
    }
    render() {
