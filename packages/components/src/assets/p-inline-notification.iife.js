...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/inline-notification/output/lit/InlineNotification.ts
  var LitInlineNotification = class extends i4 {
    get cssText() {
      ...
      let out = ":host{display:block}:host([hidden]){display:none !important}:not(:defined,[data-ssr]){visibility:hidden}";
      if (hasHeading) {
      ...
      out += 'slot:not([name]),slot[name="description"],p{all:unset;grid-area:' + descGrid + ";margin-top:" + descMargin + ";font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);color:var(--p-color-primary)}.notification{display:grid;grid-template:repeat(3, auto) / auto minmax(0, 1fr) repeat(2, auto);padding:calc(var(--p-spacing-static-sm) + var(--p-spacing-fluid-sm));border-radius:var(--p-radius-2xl);background:" + bg + ";-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted)}";
      if (dismiss) {
      ...
      if (hasAction) {
        out += ".action{grid-area:3/1/auto/-1;margin-top:var(--p-spacing-static-md);align-self:flex-start}";
      }
      ...
      if (hasAction) {
        out += ".action{grid-area:1/3;margin-top:0px;margin-inline-start:var(--p-spacing-static-md)}";
      }
      out += "}";
    ...
    get descriptionNode() {
      ...
      return description ? b2`<p>${description}</p>` : b2`<slot></slot>`;
    }
    get actionNode() {
      if (!this.hasAction) return A;
      return b2`<p-button-pure class="action" icon=${this.actionIconName || A} loading=${this.actionLoadingFlag || A}>${this.actionLabelText}</p-button-pure>`;
    }
    get dismissNode() {
      if (!this.showDismiss) return A;
      return b2`<button class="dismiss" type="button"><span>Close notification</span></button>`;
    }
    render() {
      return b2`<div class="notification" role=${this.roleName} aria-live=${this.ariaLive} aria-label=${this.headingAria || A}><style .innerHTML="${this.cssText}"></style>${this.headingNode}${this.descriptionNode}${this.actionNode}${this.dismissNode}</div>`;
    }
  };
  LitInlineNotification.styles = i`
      :host {
          display: block;
        }
        :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitInlineNotification.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitInlineNotification.prototype, "heading", 2);
  __decorateClass([
    n4({ attribute: "action-label" })
  ], LitInlineNotification.prototype, "actionLabel", 2);
  __decorateClass([
    n4({ attribute: "dismiss-button" })
  ], LitInlineNotification.prototype, "dismissButton", 2);
  __decorateClass([
    n4({ attribute: "heading-tag" })
  ], LitInlineNotification.prototype, "headingTag", 2);
  __decorateClass([
    n4()
  ], LitInlineNotification.prototype, "description", 2);
  __decorateClass([
    n4({ attribute: "action-icon" })
  ], LitInlineNotification.prototype, "actionIcon", 2);
  __decorateClass([
    n4({ attribute: "action-loading" })
  ], LitInlineNotification.prototype, "actionLoading", 2);
  LitInlineNotification = __decorateClass([
    t3("p-inline-notification")
  ], LitInlineNotification);
})();
