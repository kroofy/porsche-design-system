...
import { PModal } from '../../../../projects/angular-wrapper/src/elements/PModal';
...
describe('PModal', () => {
  it('returns a dialog with nothing wrapping it', () => {
    @Component({
      standalone: true,
      imports: [PModal],
      template: `<dialog pModal>
        <header class="p-modal__header">Some Heading</header>
    ...
    expect(host.querySelector('p-modal')).toBeNull();
    expect(dialog.classList.contains('p-modal')).toBe(true);
  ...
  it('omits the dismiss button when dismissButton is false', () => {
    @Component({
      standalone: true,
      imports: [PModal],
      template: `<dialog pModal [dismissButton]="false">Some Content</dialog>`,
    })
    class Host {}

    const fixture = render(Host);
    expect(fixture.nativeElement.querySelector('.p-modal__dismiss')).toBeNull();
  });
  ...
  it('encodes appearance on the dialog', () => {
    @Component({
      standalone: true,
      imports: [PModal],
      template: `<dialog pModal background="surface" backdrop="shading" [fullscreen]="true"></dialog>`,
    })
    class Host {}
