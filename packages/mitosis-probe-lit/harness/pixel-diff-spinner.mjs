...
const PLAYGROUND_URL = 'http://localhost:3333/?components=spinner';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_spinner_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_spinner_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/spinner_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_spinner_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-spinner.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-spinner'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="spinner"] p-spinner')];
  for (const el of hosts) {
    const lit = document.createElement('lit-spinner');
...
await pauseCardAnimation(page);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="spinner"] lit-spinner')];
  return (
    hosts.length >= 14 &&
    hosts.every((el) => el.shadowRoot?.querySelector('svg') && el.shadowRoot.querySelectorAll('circle').length === 2)
  );
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="spinner"] lit-spinner')].filter((el) =>
      el.shadowRoot?.querySelector('svg')
    ).length
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="spinner"] lit-spinner')?.shadowRoot?.querySelector('my-fragment')
);
