...
const PLAYGROUND_URL = 'http://localhost:3333/?components=input-text';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_input_text_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_input_text_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/input_text_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_input_text_control.png`;
...
await page.waitForFunction(
  () => customElements.get('p-input-text') && customElements.get('p-icon') && customElements.get('p-spinner'),
  { timeout: 20_000 },
);
await page.waitForSelector('[data-card="input-text"] p-input-text.hydrated', { timeout: 20_000 });
await page.evaluate(() => document.fonts.ready);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="input-text"] p-input-text')];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
...
const clipOf = async () => {
  const box = await page.locator('[data-card="input-text"]').boundingBox();
  return {
    x: Math.max(0, box.x),
    y: Math.max(0, box.y),
    width: box.width,
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-text.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-input-text'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="input-text"] p-input-text')];
  for (const el of hosts) {
    const lit = document.createElement('lit-input-text');
...
await pauseCardAnimation(page);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="input-text"] lit-input-text')];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      if (!el.shadowRoot?.querySelector('input[type="text"]')) return false;
      const spinner = el.shadowRoot?.querySelector('p-spinner');
      const icon = el.shadowRoot?.querySelector('p-icon');
      if (spinner && getComputedStyle(spinner).display !== 'none') {
        return (
          spinner.tagName === 'P-SPINNER' &&
          spinner.classList.contains('hydrated') &&
          !!spinner.shadowRoot?.querySelector('svg')
        );
      }
      if (icon && getComputedStyle(icon).display !== 'none') {
        const img = icon.shadowRoot?.querySelector('img');
        return icon.tagName === 'P-ICON' && icon.classList.contains('hydrated') && img?.complete;
      }
      return true;
    })
  );
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-text"] lit-input-text')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="text"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-text"] lit-input-text')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="input-text"] lit-input-text')?.shadowRoot?.querySelector('lit-icon,lit-spinner'),
);
