...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var assignHide = (vars, bp, hidden) => {
    const p3 = bp === "base" ? "--p-it-lw" : `--p-it-lw-${bp}`;
    const d3 = bp === "base" ? "--p-it-desc" : `--p-it-desc-${bp}`;
    if (hidden) {
  ...
  var LitInputNumber = class extends i4 {
    get hostStyle() {
      ...
      const vars = {
        ...
        "--p-it-btn-pad": hasControls ? "var(--ref-p-input-slotted-padding)" : "",
        "--p-it-btn-m": hasControls ? "var(--ref-p-input-slotted-margin)" : "",
        "--p-it-font": "var(--p-font-weight-normal) var(--p-typescale-sm) / calc(var(--p-leading-normal) + 6px) var(--p-font-porsche-next)"
      };
  ...
  LitInputNumber.styles = i`
      :host {
          ...
          --ref-p-input-slotted-padding: calc(
            11.2px * (var(--_p-input-base-a) - 0.64285714)
          ) !important;
          --ref-p-input-slotted-margin: calc(
            -1 * calc(11.2px * (var(--_p-input-base-a) - 0.64285714))
          ) !important;
        }
        :host([hidden]) {
        ...
        .button {
          padding: var(--p-it-btn-pad);
          margin: var(--p-it-btn-m);
        }
        :host(:not([controls])) .button {
          display: none;
        }
        .loading {
          position: absolute;
        ...
        @media (hover: hover) {
          :host(:not([disabled]):not([read-only]):not([loading]))
            .wrapper:hover:not(.button:hover),
          :host(:not([disabled]):not([read-only]):not([loading]))
            .label-wrapper:hover
            ~ .wrapper {
            border-color: var(--p-it-hover);
        ...
        @media (min-width: 760px) {
          .label-wrapper {
            min-width: var(
              --p-it-lw-s-minw,
              var(--p-it-lw-xs-minw, var(--p-it-lw-minw))
            );
            position: var(
              --p-it-lw-s-pos,
              var(--p-it-lw-xs-pos, var(--p-it-lw-pos, static))
            );
            ...
            overflow: var(
              --p-it-lw-s-ov,
              var(--p-it-lw-xs-ov, var(--p-it-lw-ov, visible))
            );
            clip: var(
              --p-it-lw-s-clip,
              var(--p-it-lw-xs-clip, var(--p-it-lw-clip, auto))
            );
            white-space: var(
              --p-it-lw-s-ws,
              var(--p-it-lw-xs-ws, var(--p-it-lw-ws, normal))
            );
          }
          .label:is(span) {
            position: var(
              --p-it-desc-s-pos,
              var(--p-it-desc-xs-pos, var(--p-it-desc-pos, static))
            );
            width: var(
              --p-it-desc-s-w,
              var(--p-it-desc-xs-w, var(--p-it-desc-w, auto))
            );
            height: var(
              --p-it-desc-s-h,
              var(--p-it-desc-xs-h, var(--p-it-desc-h, auto))
            );
            padding: var(
              --p-it-desc-s-pad,
              var(--p-it-desc-xs-pad, var(--p-it-desc-pad, 0))
            );
            margin: var(--p-it-desc-s-m, var(--p-it-desc-xs-m, var(--p-it-desc-m, 0)));
            overflow: var(
              --p-it-desc-s-ov,
              var(--p-it-desc-xs-ov, var(--p-it-desc-ov, visible))
            );
            clip: var(
              --p-it-desc-s-clip,
              var(--p-it-desc-xs-clip, var(--p-it-desc-clip, auto))
            );
            white-space: var(
              --p-it-desc-s-ws,
              var(--p-it-desc-xs-ws, var(--p-it-desc-ws, normal))
            );
            margin-top: calc(-1 * var(--p-spacing-static-xs));
          }
        }
