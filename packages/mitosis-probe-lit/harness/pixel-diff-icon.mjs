...
const PLAYGROUND_URL = 'http://localhost:3333/?components=icon';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_icon_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_icon_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/icon_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_icon_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-icon.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-icon');
...
await page.waitForFunction(() => {
  const icons = [...document.querySelectorAll('[data-card="icon"] lit-icon')];
  return (
    icons.length >= 15 &&
    icons.every((el) => {
      const img = el.shadowRoot?.querySelector('img');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="icon"] lit-icon')].filter((el) =>
      el.shadowRoot?.querySelector('img')
    ).length
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="icon"] lit-icon')?.shadowRoot?.querySelector('my-fragment')
);
