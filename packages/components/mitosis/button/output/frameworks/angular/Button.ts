...
export default class LitButton {
  ...
  @Input() icon!: LitButtonProps["icon"];
  @Input() iconSource!: LitButtonProps["iconSource"];
  @Input() disabled!: LitButtonProps["disabled"];
  @Input() loading!: LitButtonProps["loading"];
  ...
  get cssText() {
    ...
    const iconMarginFor = (h: any) =>
      isTrue(h)
        ? "0"
        : "calc(-1 * (11.2px * (var(--_p-button-a) - 0.64285714) + 4px))";
    const colors: any = {
      primary: {
...
@NgModule({
  declarations: [LitButton],
  imports: [CommonModule],
  exports: [LitButton],
})
export class LitButtonModule {}
