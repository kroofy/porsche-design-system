...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/multi-select/output/lit/MultiSelect.ts
  var LitMultiSelect = class extends i4 {
    get cssText() {
      const minWidth = {
        xs: 480,
        s: 760,
      ...
      let out = "@keyframes fade-in{from{opacity:0}to{opacity:1}}:host{display:block;--_p-multi-select-a:" + scale + " !important;--_p-multi-select-option-a:" + scale + " !important;--_p-optgroup-a:" + scale + ' !important}:host([hidden]){display:none !important}slot[name="label-after"]{display:inline-block;vertical-align:top}slot[name="label-after"]::slotted(*){margin-inline-start:var(--p-spacing-static-xs) !important}.label-after{display:inline-block;vertical-align:top}:not(:defined,[data-ssr]){visibility:hidden}button{all:unset;display:flex;align-items:center;gap:calc(22.4px * (var(--_p-multi-select-a) - 0.64285714) + 4px);height:calc(var(--_p-multi-select-a) * 3.5rem);box-sizing:border-box;min-width:0;padding-inline:calc(22.4px * (var(--_p-multi-select-a) - 0.64285714) + 8px);border:1px solid var(--p-multi-select-border-color,' + palette.border + ");border-radius:" + (compact ? "var(--p-radius-lg)" : "var(--p-radius-xl)") + ";background:var(--p-multi-select-background-color," + palette.bg + ");font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);color:var(--p-multi-select-text-color,var(--p-color-primary));cursor:" + (disabled ? "not-allowed" : "pointer");
      if (disabled) out += ";opacity:0.4";
      out += ";transition:background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), border-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out)}";
      if (!disabled) {
        out += "button:focus-visible{outline:2px solid var(--p-color-focus);outline-offset:2px}@media(forced-colors:active){button:focus-visible{outline-color:Highlight}}@media(hover:hover){button:hover,label:hover~button{border-color:var(--p-multi-select-border-color," + palette.hover + ")}}";
      }
      if (disabled) {
        out += "@media(forced-colors:active){button{opacity:1;color:GrayText}}";
      }
      out += 'button span{flex-grow:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}[popover]{all:unset;position:absolute;z-index:99;padding:calc(11.2px * (var(--_p-multi-select-a) - 0.64285714) + 4px);display:none;flex-direction:column;gap:calc(11.2px * (var(--_p-multi-select-a) - 0.64285714) + 4px);max-height:max(calc(242px), calc(50vh - 54px / 2 - 6px * 2));box-sizing:border-box;overflow:hidden auto;scrollbar-width:thin;scrollbar-color:auto;animation:var(--p-animation-duration,var(--p-duration-sm)) fade-in var(--p-ease-in-out) forwards;filter:drop-shadow(0 0 8px rgba(0,0,0,0.15));background:var(--p-color-canvas);border:1px solid var(--p-color-contrast-low);border-radius:var(--p-radius-xl)}[popover]:not(:popover-open){display:none}slot[name="selected"]{display:block;height:100%;flex-grow:1;overflow:hidden}.root{display:grid;gap:var(--p-spacing-static-xs);min-width:calc(1rem + var(--p-spacing-static-md) + 1px * 2 + calc(9px + var(--p-spacing-static-md) / 2 + (var(--p-leading-normal) + var(--p-spacing-static-xs) * 2) * 2))}.options{display:flex;flex-direction:column;gap:calc(11.2px * (var(--_p-multi-select-a) - 0.64285714) + 4px)}.icon{margin-inline-end:-3px;pointer-events:none;transform:rotate3d(0,0,1,0.0001deg);transition:transform var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out)}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}';
      out += ".label-wrapper{" + labelVisFor(hideBase) + "}";
      out += ".label{font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);cursor:" + (disabled ? "not-allowed" : "pointer") + ";color:var(--p-color-primary)";
    ...
    get isDisabled() {
      return (this.disabled ?? this.getAttribute("disabled")) === true || (this.disabled ?? this.getAttribute("disabled")) === "true" || (this.disabled ?? this.getAttribute("disabled")) === "";
    }
    get isRequired() {
      return (this.required ?? this.getAttribute("required")) === true || (this.required ?? this.getAttribute("required")) === "true" || (this.required ?? this.getAttribute("required")) === "";
    }
    get ariaInvalid() {
    ...
    get labelNode() {
      if (!this.hasLabel) return A;
      return b2`<div class="label-wrapper"><label class="label" id="label" for="button" aria-disabled=${this.isDisabled ? "true" : A}>${this.labelText}<slot name="label"></slot></label><slot name="label-after"></slot></div>`;
    }
    get descriptionNode() {
      if (!this.hasDescription) return A;
      return b2`<span class="label" id="description" aria-disabled=${this.isDisabled ? "true" : A}>${this.descriptionText}<slot name="description"></slot></span>`;
    }
    get iconNode() {
      ...
      return b2`<p-icon name=${icon} source=${src} color=${this.iconColor || A} aria-hidden="true"></p-icon>`;
    }
    itemChildren() {
      return [...this.children].filter(
        (el) => el.slot !== "label" && el.slot !== "label-after" && el.slot !== "description" && el.slot !== "message" && el.slot !== "filter" && el.slot !== "selected"
      );
    }
    selectedLabel() {
    ...
    render() {
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style>${this.labelNode}${this.descriptionNode}<button type="button" role="combobox" id="button" tabindex="0" aria-haspopup="listbox" aria-expanded="false" aria-required=${this.isRequired ? "true" : "false"} aria-controls="listbox" aria-autocomplete="none" aria-labelledby=${this.hasLabel ? "label" : A} aria-describedby=${this.comboDescribedBy()} aria-invalid=${this.ariaInvalid || A} ?disabled=${!!this.isDisabled}><span>${this.selectedLabel()}</span><p-icon class="icon" name="arrow-head-down" source="http://localhost:3001/icons/arrow-head-down.1e3cbb8.svg" color="primary" aria-hidden="true"></p-icon></button><div popover="manual" tabindex="0"><div id="listbox" class="options" role="listbox" aria-labelledby=${this.hasLabel ? "label" : A} aria-required=${this.isRequired ? "true" : "false"} aria-multiselectable="true" tabindex="-1"><slot></slot></div></div><span class="message" id="message" role=${this.messageRole}>${this.iconNode}${this.messageText}</span></div>`;
    }
  };
  LitMultiSelect.styles = i`
      :host([hidden]) {
          display: none !important;
        }
`;
  __decorateClass([
    n4({ attribute: "dropdown-direction" })
  ], LitMultiSelect.prototype, "dropdownDirection", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "form", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "name", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "value", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "disabled", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "compact", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "message", 2);
  __decorateClass([
    n4({ attribute: "hide-label" })
  ], LitMultiSelect.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "label", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "description", 2);
  __decorateClass([
    n4()
  ], LitMultiSelect.prototype, "required", 2);
  LitMultiSelect = __decorateClass([
    t3("p-multi-select")
  ], LitMultiSelect);
})();
