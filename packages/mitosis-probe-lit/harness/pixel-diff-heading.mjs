...
const PLAYGROUND_URL = 'http://localhost:3333/?components=heading';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_heading_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_heading_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/heading_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_heading_control.png`;
...
await page.waitForSelector('[data-card="heading"] p-heading.hydrated', { timeout: 20_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-heading.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-heading'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="heading"] p-heading')];
  for (const el of hosts) {
    const lit = document.createElement('lit-heading');
...
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="heading"] lit-heading')];
  return (
    hosts.length >= 12 &&
    hosts.every((el) => {
      const slot = el.shadowRoot?.querySelector('slot');
      return !!el.shadowRoot?.querySelector('h2') && (slot?.assignedNodes({ flatten: true }).length ?? 0) > 0;
    })
  );
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="heading"] lit-heading')].filter((el) =>
      el.shadowRoot?.querySelector('h2')
    ).length
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="heading"] lit-heading')?.shadowRoot?.querySelector('my-fragment')
);
