...
@customElement("p-segmented-control-item")
...
`;
  ...
  render() {
    return html`<button type="button" aria-pressed=${this.isSelected ? "true" : "false"} aria-disabled=${this.isDisabled ? "true" : nothing}><style .innerHTML="${this.cssText}"></style>${this.labelNode}${this.iconNode}<slot></slot></button>`;
  }
}
