...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitTableHeadCell = class extends i4 {
    ...
    render() {
      if (this.sortable) {
        return b2`<button type="button" @click=${this.onButtonClick}><slot></slot><p-icon class="icon" color="inherit" size="x-small" name="arrow-up" aria-hidden="true"></p-icon></button>`;
      }
      return b2`<span><slot></slot></span>`;
    }
  };
  LitTableHeadCell.styles = i`
      :host {
          display: table-cell;
          padding: 2px var(--_p-table-a, var(--p-spacing-fluid-sm))
            var(--_p-table-a, var(--p-spacing-fluid-sm)) !important;
