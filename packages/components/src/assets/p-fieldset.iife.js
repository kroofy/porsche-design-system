...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitFieldset = class extends i4 {
    get hostStyle() {
      ...
      const small = ((this.getAttribute("label-size") ?? this.labelSize) || "medium") === "small";
      const palettes = {
        none: "",
      ...
      return {
        ...
        "--p-fieldset-legend-font": small ? "var(--p-font-weight-semibold) var(--p-typescale-sm)" : "var(--p-font-weight-normal) var(--p-typescale-md)",
    ...
    render() {
      return b2`<fieldset><legend>${this.labelText}</legend><slot></slot><span class="message" id="message"><p-icon name=${this.iconName || A} source=${this.iconSrc || A} color=${this.iconColor || A} aria-hidden="true"></p-icon>${this.messageText}</span></fieldset>`;
    }
  };
  LitFieldset.styles = i`
      :host {
        ...
        legend {
          ...
          font: var(--p-fieldset-legend-font) / var(--p-leading-normal)
