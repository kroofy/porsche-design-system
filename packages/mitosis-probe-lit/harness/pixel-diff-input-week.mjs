...
const hostReady = () => {
  const hosts = [
    ...document.querySelectorAll('[data-card="input-week"] p-input-week, [data-card="input-week"] lit-input-week'),
  ];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('input[type="week"]')) return false;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-week.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-input-week');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-week"] lit-input-week')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="week"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-week"] lit-input-week')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="input-week"] lit-input-week')
