...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitCanvas = class extends i4 {
    ...
    render() {
      ...
      const headerToggle = startOpen ? A : b2`<p-button icon="sidebar" variant="secondary" compact="true" hide-label="true" aria='{"aria-expanded":false}' @click=${() => this._toggleStart()}>Open navigation sidebar</p-button>`;
      const titleNode = hasTitle ? b2`<h2><slot name="title"></slot></h2>` : A;
      const sidebarEndNode = hasSidebarEnd ? b2`<aside class="sidebar sidebar--end" ?inert=${!endOpen} aria-label=${endOpen ? "Settings sidebar open" : "Settings sidebar closed"} tabindex="-1"><div class="sidebar__header sidebar__header--end"><slot name="sidebar-end-header"></slot><p-button icon="close" variant="secondary" compact="true" hide-label="true" aria=${endOpen ? '{"aria-expanded":true}' : '{"aria-expanded":false}'} @click=${() => this._dismissEnd()}>${endOpen ? "Close" : "Open"} settings sidebar</p-button></div><slot name="sidebar-end"></slot></aside>` : A;
