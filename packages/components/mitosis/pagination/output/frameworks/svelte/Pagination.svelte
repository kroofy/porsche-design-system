...
<script lang="ts">
  ...
  $: cssText = () => {
    const totalItems = Number(
      totalItemsCount == null || totalItemsCount === "" ? 1 : totalItemsCount
    );
    const perPage = Number(
      itemsPerPage == null || itemsPerPage === "" ? 1 : itemsPerPage
    );
    const pageTotal = Math.ceil(
      (totalItems < 1 ? 1 : totalItems) / (perPage < 1 ? 1 : perPage)
    );
    let active = Number(
      activePage == null || activePage === "" ? 1 : activePage
    );
    if (active < 1) active = 1;
    if (active > pageTotal) active = pageTotal;
    let showLast: any = showLastPage;
