...
const BASELINE_PNG =
  ...
  '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_table_head_row_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_table_head_row_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/table_head_row_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_table_head_row_control.png`;
...
const hostReady = () => {
  ...
  const rows = [
    ...document.querySelectorAll('[data-card="table"] p-table-head-row, [data-card="table"] lit-table-head-row'),
  ];
  return (
    parents.length >= 2 &&
    rows.length >= 2 &&
    ...
    rows.every((el) => {
      ...
      if (el.tagName === 'P-TABLE-HEAD-ROW' && !el.classList.contains('hydrated')) return false;
...
await page.waitForSelector('[data-card="table"] p-table-head-row.hydrated', {
  timeout: 20_000,
  state: 'attached',
});
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-table-head-row.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-table-head-row'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="table"] p-table-head-row')];
  for (const el of hosts) {
    const lit = document.createElement('lit-table-head-row');
...
await page.evaluate(async () => {
  await Promise.all(
    [...document.querySelectorAll('[data-card="table"] lit-table-head-row')].map((el) => el.updateComplete),
  );
  await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="table"] lit-table-head-row')].filter((el) =>
      el.shadowRoot?.querySelector('slot'),
    ).length,
);
swap.fragment = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="table"] lit-table-head-row')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="table"] lit-table-head-row')
      ?.shadowRoot?.querySelector('lit-table-head-cell,lit-table-head,lit-table'),
);
swap.parentStillStencil = await page.evaluate(() =>
  [...document.querySelectorAll('[data-card="table"] lit-table-head-row')].every(
    (el) => el.parentElement?.tagName === 'P-TABLE-HEAD',
  ),
);
swap.slottedCopied = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="table"] lit-table-head-row')].every((el) =>
      el.querySelector(':scope > p-table-head-cell'),
    ),
);
swap.nested = await page.evaluate(() => {
  const rows = [...document.querySelectorAll('[data-card="table"] lit-table-head-row')];
  const cells = [...document.querySelectorAll('[data-card="table"] lit-table-head-row > p-table-head-cell')];
  const heads = [...document.querySelectorAll('[data-card="table"] p-table-head')];
  return {
    rowCount: rows.length,
    cellTags: [...new Set(cells.map((n) => n.tagName))],
    parentTags: [...new Set(heads.map((n) => n.tagName))],
  };
});
...
const failed =
  ...
  (swap.nested && swap.nested.parentTags.some((tag) => tag !== 'P-TABLE-HEAD')) ||
