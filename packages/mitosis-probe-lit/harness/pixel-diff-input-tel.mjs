...
const PLAYGROUND_URL = process.env.PLAYGROUND_URL ?? 'http://localhost:3333/?components=input-tel';
const BASELINE_PNG =
  process.env.BASELINE_PNG ?? '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_input_tel_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_input_tel_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/input_tel_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_input_tel_control.png`;
...
const hostReady = () => {
  const hosts = [...document.querySelectorAll('[data-card="input-tel"] p-input-tel, [data-card="input-tel"] lit-input-tel')];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('input[type="tel"]')) return false;
...
await page.waitForFunction(
  () => customElements.get('p-input-tel') && customElements.get('p-icon') && customElements.get('p-spinner'),
  { timeout: 20_000 },
);
await page.waitForSelector('[data-card="input-tel"] p-input-tel.hydrated', { timeout: 20_000 });
await page.evaluate(() => document.fonts.ready);
await page.waitForFunction(hostReady);
await pauseCardAnimation(page);

const clipOf = async () => {
  const box = await page.locator('[data-card="input-tel"]').boundingBox();
  return {
    x: Math.max(0, box.x),
    y: Math.max(0, box.y),
    width: box.width,
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-tel.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-input-tel'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="input-tel"] p-input-tel')];
  for (const el of hosts) {
    const lit = document.createElement('lit-input-tel');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-tel"] lit-input-tel')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="tel"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-tel"] lit-input-tel')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="input-tel"] lit-input-tel')?.shadowRoot?.querySelector('lit-icon,lit-spinner'),
);
