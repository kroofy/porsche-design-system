...
const BASELINE_PNG =
  ...
  '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_table_body_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_table_body_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/table_body_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_table_body_control.png`;
...
const hostReady = () => {
  ...
  const bodies = [
    ...document.querySelectorAll('[data-card="table"] p-table-body, [data-card="table"] lit-table-body'),
  ];
  return (
    parents.length >= 2 &&
    bodies.length >= 2 &&
    parents.every((el) => el.classList.contains('hydrated') && el.shadowRoot?.querySelector('.table')) &&
    bodies.every((el) => {
      if (el.parentElement?.tagName !== 'P-TABLE') return false;
      if (el.tagName === 'P-TABLE-BODY' && !el.classList.contains('hydrated')) return false;
      if (!el.shadowRoot?.querySelector('slot')) return false;
      const rows = [...el.querySelectorAll(':scope > p-table-row')];
      const cells = [...el.querySelectorAll('p-table-cell')];
      return (
        rows.length > 0 &&
...
await page.waitForSelector('[data-card="table"] p-table-body.hydrated', {
  timeout: 20_000,
  state: 'attached',
});
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-table-body.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-table-body'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="table"] p-table-body')];
  for (const el of hosts) {
    const lit = document.createElement('lit-table-body');
...
await page.evaluate(async () => {
  await Promise.all(
    [...document.querySelectorAll('[data-card="table"] lit-table-body')].map((el) => el.updateComplete),
  );
  await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="table"] lit-table-body')].filter((el) =>
      el.shadowRoot?.querySelector('slot'),
    ).length,
);
swap.fragment = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="table"] lit-table-body')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="table"] lit-table-body')
      ?.shadowRoot?.querySelector('lit-table-row,lit-table-cell,lit-table'),
);
swap.parentStillStencil = await page.evaluate(() =>
  [...document.querySelectorAll('[data-card="table"] lit-table-body')].every(
    (el) => el.parentElement?.tagName === 'P-TABLE',
  ),
);
swap.slottedCopied = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="table"] lit-table-body')].every((el) =>
      el.querySelector(':scope > p-table-row'),
    ),
);
swap.nested = await page.evaluate(() => {
  const bodies = [...document.querySelectorAll('[data-card="table"] lit-table-body')];
  const rows = [...document.querySelectorAll('[data-card="table"] lit-table-body > p-table-row')];
  const cells = [...document.querySelectorAll('[data-card="table"] lit-table-body p-table-cell')];
  ...
  return {
    ...
    parentTags: [...new Set(tables.map((n) => n.tagName))],
  };
});
...
const failed =
  ...
  !swap.slottedCopied ||
  swap.swapped < 2 ||
  (swap.nested && swap.nested.rowTags.some((tag) => tag !== 'P-TABLE-ROW')) ||
  (swap.nested && swap.nested.cellTags.some((tag) => tag !== 'P-TABLE-CELL')) ||
  (swap.nested && swap.nested.parentTags.some((tag) => tag !== 'P-TABLE')) ||
