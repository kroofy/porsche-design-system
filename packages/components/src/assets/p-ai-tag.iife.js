...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitAiTag = class extends i4 {
    ...
    render() {
      return this.isAbbreviation ? b2`<div><abbr title=${this.longLabel}>${this.shortLabel}</abbr></div>` : b2`<div>${this.copyLabel}</div>`;
    }
  };
  LitAiTag.styles = i`
      :host {
        ...
        div {
          ...
          border-radius: calc(
            var(--p-spacing-static-xs) + (var(--p-leading-normal) / 2)
          );
          ...
          transition: color var(--p-transition-duration, var(--p-duration-sm))
              var(--p-ease-in-out),
            background-color var(--p-transition-duration, var(--p-duration-sm))
              var(--p-ease-in-out),
            backdrop-filter var(--p-transition-duration, var(--p-duration-sm))
              var(--p-ease-in-out);
        }
        div::before {
          content: "";
