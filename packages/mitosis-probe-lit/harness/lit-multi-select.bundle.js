...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitMultiSelect = class extends i4 {
    ...
    render() {
      ...
      const labelBlock = hasLabel ? b2`<div class="label-wrapper"><label class="label" id="label" for="button" aria-disabled=${disabled ? "true" : A}>${this.labelText}<slot name="label"></slot></label><slot name="label-after"></slot></div>` : A;
      const descBlock = hasDescription ? b2`<span class="label" id="description" aria-disabled=${disabled ? "true" : A}>${this.descriptionText}<slot name="description"></slot></span>` : A;
      const icon = hasMessage ? b2`<p-icon name=${this.iconName} color=${this.iconColor || A} aria-hidden="true"></p-icon>` : A;
      return b2`<div class="root"><style .innerHTML="${this.cssText}"></style>${labelBlock}${descBlock}<button type="button" role="combobox" id="button" tabindex="0" aria-haspopup="listbox" aria-expanded="false" aria-required=${required ? "true" : "false"} aria-controls="listbox" aria-autocomplete="none" aria-labelledby=${hasLabel ? "label" : A} aria-describedby=${this.comboDescribedBy()} aria-invalid=${this.ariaInvalid || A} ?disabled=${disabled}><span>${this.selectedLabel()}</span><p-icon class="icon" name="arrow-head-down" color="primary" aria-hidden="true"></p-icon></button><div popover="manual" tabindex="0"><div id="listbox" class="options" role="listbox" aria-labelledby=${hasLabel ? "label" : A} aria-required=${required ? "true" : "false"} aria-multiselectable="true" tabindex="-1"><slot></slot></div></div><span class="message" id="message" role=${this.messageRole}>${icon}${this.messageText}</span></div>`;
    }
  };
