...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/modal/output/lit/Modal.ts
  var LitModal = class extends i4 {
    get cssText() {
      const isTrue = (v2) => v2 === true || v2 === "true" || v2 === "";
      const isOpen = isTrue(this.open ?? this.getAttribute("open"));
      let dismiss = this.dismissButton ?? this.getAttribute("dismiss-button") ?? this.getAttribute("dismissbutton");
      ...
      const panelClosed = isOpen ? "opacity:1;transform:translate3d(0,0,0)" : "opacity:0;transform:translate3d(0,25vh,0)";
      const centered = "width:var(--p-modal-width,auto);min-width:276px;max-width:1535.5px;place-self:center;margin:var(--p-modal-spacing-top,clamp(16px, 10vh, 192px)) max(22px, 10.625vw - 12px) var(--p-modal-spacing-bottom,clamp(16px, 10vh, 192px));border-radius:var(--p-radius-3xl);clip-path:inset(0 round var(--p-radius-3xl))";
      const stretched = "width:auto;min-width:auto;max-width:none;place-self:stretch;margin:0;border-radius:0;clip-path:none";
      const minWidth = {
        xs: 480,
        s: 760,
        m: 1e3,
      ...
      const fsRule = (v2) => v2 === true || v2 === "true" ? stretched : centered;
      ...
      if (typeof fullscreen === "object" && fullscreen !== null) {
        ...
        for (const bp of Object.keys(fullscreen)) {
          ...
          modalMedia += "@media(min-width:" + minWidth[bp] + "px){.modal{" + fsRule(fullscreen[bp]) + "}}";
        }
      } else {
        modalBox = fsRule(fullscreen);
      }
      const hcmOutline = isTrue(fullscreen) ? "" : "@media(forced-colors:active){.modal{outline:2px solid CanvasText;outline-offset:-2px}}";
      let out = ":host{display:contents;--ref-p-modal-pt:var(--p-spacing-fluid-md) !important;--ref-p-modal-pb:calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) !important;--ref-p-modal-px:var(--p-spacing-fluid-lg) !important;--pds-internal-grid-outer-column:calc(var(--p-spacing-fluid-lg) - clamp(16px, 1.25vw + 12px, 36px)) !important;--pds-internal-grid-margin:calc(var(--p-spacing-fluid-lg) * -1) !important;--pds-internal-grid-width-min:auto !important;--pds-internal-grid-width-max:none !important;--_p-dialog-a:" + dialogBg + ' !important}:host([hidden]){display:none !important}slot{display:block}slot:first-of-type{grid-row-start:1}slot:not([name]){grid-column:2/3;z-index:0}slot[name=header]{grid-column:2/3;z-index:0}slot[name=footer]{grid-column:1/-1;z-index:2;position:sticky;bottom:-.1px;margin-block:calc(-1 * calc(calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) - var(--p-radius-3xl)));padding:calc(calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) - var(--p-radius-3xl)) var(--p-spacing-fluid-lg);background:linear-gradient(0deg,var(--_p-dialog-a) 0%,var(--_p-dialog-a) 20%,transparent 80%)}slot[name=footer][data-stuck]::after{content:"";z-index:-1;position:absolute;inset:calc(calc(calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) - var(--p-radius-3xl)) - 12 * var(--p-spacing-static-2xs)) calc(var(--p-spacing-fluid-lg) - 12 * var(--p-spacing-static-2xs));background:var(--p-color-frosted);border-radius:var(--p-radius-2xl);-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted)}dialog{all:unset;position:fixed;inset:0;max-width:100dvw;max-height:100dvh;overflow:hidden;display:block;user-select:text;outline:0;' + dialogVis + ";transition:" + dialogTrans + ";overlay:none}dialog:modal{overlay:auto}dialog::backdrop{display:none}@supports (overlay: auto) and (transition-behavior: allow-discrete){dialog{transition:" + dialogTrans + ", overlay " + dialogDur + " " + ease + " allow-discrete}}.scroller{position:absolute;isolation:isolate;display:grid;inset:0;overflow:hidden auto;overscroll-behavior-y:none;background:rgba(255,255,255,.01);transform:translate3d(0,0,0)}.modal{position:relative;display:grid;grid-template:auto/var(--p-spacing-fluid-sm) minmax(0,1fr) var(--p-spacing-fluid-sm);gap:var(--p-spacing-fluid-md) calc(var(--p-spacing-fluid-lg) - var(--p-spacing-fluid-sm));padding-top:var(--p-spacing-fluid-md);padding-bottom:calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md));align-content:flex-start;color:var(--p-color-primary);background:var(--_p-dialog-a);" + panelClosed + ";transition:opacity " + panelDur + " " + ease + ", transform " + panelDur + " " + ease + ";" + modalBox + "}" + modalMedia + (isOpen ? "" : ".modal:dir(rtl){transform:translate3d(0,25vh,0)}") + hcmOutline;
      if (dismiss) {
        out += ".dismiss{all:unset;box-sizing:border-box;display:grid;place-items:center;padding:6px;border-radius:var(--p-radius-full);font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);background-color:" + dismissBg + ";color:var(--p-color-primary);cursor:pointer;transition:background-color " + durSm + " var(--p-ease-in-out), color " + durSm + ' var(--p-ease-in-out);grid-area:1/3;z-index:5;position:sticky;top:var(--p-spacing-fluid-sm);margin-top:calc(-1 * var(--p-spacing-fluid-md) + var(--p-spacing-fluid-sm));margin-inline-end:var(--p-spacing-fluid-sm);place-self:flex-start flex-end}.dismiss:focus-visible{outline:2px solid var(--p-color-focus);outline-offset:2px}.dismiss::before{content:"";width:var(--p-leading-normal);height:var(--p-leading-normal);-webkit-mask:' + closeMask + ";mask:" + closeMask + ";background:currentColor}.dismiss span{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}@media(forced-colors:active){.dismiss{forced-color-adjust:none;background:Canvas;box-shadow:inset 0 0 0 2px ButtonBorder;color:ButtonText}.dismiss:focus-visible{outline-color:Highlight}}@media(hover:hover){.dismiss:hover{background-color:" + dismissHover + "}@media(forced-colors:active){.dismiss:hover{background:Canvas}}}";
      }
      return out;
    }
    get isOpenFlag() {
    ...
    render() {
      const dismiss = this.showDismiss ? b2`<button class="dismiss" type="button"><span>Dismiss modal</span></button>` : A;
      const label = this.ariaLabelText || A;
      return b2`<style .innerHTML="${this.cssText}"></style><dialog ?inert=${!this.isOpenFlag} tabindex="-1" aria-modal="true" aria-label=${label}><div class="scroller"><div class="modal">${dismiss}<slot name="header"></slot><slot></slot><slot name="footer"></slot></div></div></dialog>`;
    }
  };
  LitModal.styles = i`
      :host {
...
`;
  __decorateClass([
    n4()
  ], LitModal.prototype, "open", 2);
  __decorateClass([
    n4()
  ], LitModal.prototype, "dismissButton", 2);
  __decorateClass([
    n4()
  ], LitModal.prototype, "background", 2);
  __decorateClass([
    n4()
  ], LitModal.prototype, "backdrop", 2);
  __decorateClass([
    n4()
  ], LitModal.prototype, "fullscreen", 2);
  __decorateClass([
    n4()
  ], LitModal.prototype, "aria", 2);
  LitModal = __decorateClass([
    t3("p-modal")
  ], LitModal);
})();
