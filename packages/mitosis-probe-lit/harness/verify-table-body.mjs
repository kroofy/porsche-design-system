...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/table-body.html');
await page.waitForFunction(() => customElements.get('lit-table-body'));
await page.waitForFunction(() =>
  document.querySelector('#default lit-table-body')?.shadowRoot?.querySelector('slot'),
);

const live = await page.evaluate(() => {
  const el = document.querySelector('#default lit-table-body');
  const sr = el.shadowRoot;
  const style = sr?.querySelector('style');
  const slot = sr?.querySelector('slot:not([name])');
  const css = style?.textContent ?? '';
  return {
    isDefined: !!customElements.get('lit-table-body'),
    hasShadowRoot: !!sr,
    dynamicStyleTag: !!style,
    cssHasRowGroup: css.includes(':host{display:table-row-group'),
    ...
    innerLit: !!sr?.querySelector('lit-table-row,lit-table-cell,lit-table'),
  };
});

const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-table-body')).display);

await browser.close();

const failures = [];
if (!live.isDefined) failures.push('lit-table-body not defined');
if (!live.hasShadowRoot) failures.push('missing shadowRoot');
if (!live.dynamicStyleTag) failures.push('missing cssText style tag');
if (!live.cssHasRowGroup) failures.push('css missing table-row-group');
...
if (live.hostDisplay !== 'table-row-group') failures.push('host display');
