...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitSegmentedControlItem = class extends i4 {
    ...
    get labelNode() {
      ...
      return label ? b2`<span>${label}</span>` : A;
    }
    get iconNode() {
      const icon = this.iconName;
      const source = this.iconSrc;
      if (!icon && !source) return A;
      return b2`<p-icon class="icon" name=${icon || A} source=${source || A} color="inherit" size="inherit" aria-hidden="true"></p-icon>`;
    }
    connectedCallback() {
      ...
      this.addEventListener("blur", (e5) => {
        e5.stopPropagation();
        const related = e5.relatedTarget;
