...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-password.html');
await page.waitForFunction(() => customElements.get('lit-input-password'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-password');
  return !!el?.shadowRoot?.querySelector('input[type="password"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-password'),
    hasShadowRoot: !!sr,
    ...
    toggleIsLit: toggle?.tagName === 'LIT-BUTTON-PURE',
    toggleIcon: toggle?.getAttribute('icon'),
    toggleClass: toggle?.getAttribute('class'),
    toggleHidden: toggle ? getComputedStyle(toggle).display === 'none' : true,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-password');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-password');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-password');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-password');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-password');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-password');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-password')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-password');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-password is not defined');
...
if (live.inputType !== 'password') failures.push(`input type ${live.inputType}`);
...
if (live.toggleTag !== 'P-BUTTON-PURE') failures.push(`toggle tag ${live.toggleTag}`);
if (live.toggleIsLit) failures.push('toggle was swapped to lit-button-pure');
if (live.toggleIcon !== 'view') failures.push(`toggle icon ${live.toggleIcon}`);
if (live.toggleClass !== 'button') failures.push(`toggle class ${live.toggleClass}`);
if (live.toggleHidden) failures.push('default toggle hidden');
if (live.toggleText !== 'Toggle password visibility') failures.push(`toggle text ${live.toggleText}`);
...
if (!hideLabel.toggleHidden) failures.push('hide-label host still shows toggle');
