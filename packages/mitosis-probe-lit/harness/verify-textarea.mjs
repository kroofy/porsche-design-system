...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/textarea.html');
await page.waitForFunction(() => customElements.get('lit-textarea'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-textarea');
...
const live = await page.evaluate(() => {
  ...
  const input = sr?.querySelector('textarea');
  ...
  return {
    isDefined: !!customElements.get('lit-textarea'),
    hasShadowRoot: !!sr,
    dynamicStyleTag: !!style,
    cssTextHasTextarea: !!style?.textContent?.includes('textarea{'),
    cssTextHasScale: !!style?.textContent?.includes('--_p-textarea-a:1'),
    cssTextHasCounter: !!style?.textContent?.includes('.counter{pointer-events:none'),
    unknownWrapper: sr?.querySelector('my-fragment') ? 'my-fragment' : null,
    rootClass: sr?.querySelector('.root')?.className ?? null,
    inputValue: input?.value,
    rows: input?.getAttribute('rows') ?? String(input?.rows),
    labelText: label?.textContent,
    counterText: counter?.textContent?.trim(),
    counterHidden: counter ? getComputedStyle(counter).display === 'none' : true,
    backgroundMatch: wrapper && bgRef ? getComputedStyle(input).backgroundColor === getComputedStyle(bgRef).backgroundColor : false,
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-textarea');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-textarea');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-textarea');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-textarea');
  const input = el.shadowRoot?.querySelector('textarea');
  const css = input ? getComputedStyle(input) : null;
  const ref = document.querySelector('#readonly-ref');
  return {
    borderColor: css?.borderColor,
    colorMatch: input && ref ? css.color === getComputedStyle(ref).color : false,
    inputReadonly: input?.readOnly,
  };
});

const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-textarea');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-textarea')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-textarea');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
...
const failures = [];
if (!live.isDefined) failures.push('lit-textarea is not defined');
if (!live.hasShadowRoot) failures.push('no shadowRoot');
if (!live.dynamicStyleTag) failures.push('no cssText <style>');
if (!live.cssTextHasTextarea) failures.push('cssText missing textarea rules');
if (!live.cssTextHasScale) failures.push('cssText missing --_p-textarea-a:1');
if (!live.cssTextHasCounter) failures.push('cssText missing .counter');
if (live.unknownWrapper) failures.push('my-fragment leaked');
if (live.rootClass !== 'root') failures.push(`root class ${live.rootClass}`);
if (live.inputValue !== 'Some value') failures.push(`textarea value ${live.inputValue}`);
if (live.rows !== '1') failures.push(`rows ${live.rows}`);
if (live.labelText !== 'Some label') failures.push(`label ${live.labelText}`);
if (live.counterText !== '10/100') failures.push(`counter ${live.counterText}`);
if (live.counterHidden) failures.push('default counter hidden');
...
if (!disabled.counterHidden) failures.push('disabled counter should be hidden');
...
if (!readonly.inputReadonly) failures.push('readonly textarea not readOnly');
