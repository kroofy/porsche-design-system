...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/tag/output/lit/Tag.ts
  var LitTag = class extends i4 {
    get cssText() {
      const textMap = {
      ...
      const pad = compact ? "padding:var(--p-spacing-static-2xs) var(--p-spacing-static-sm);border-radius:calc(1px + (var(--p-leading-normal) / 2));" : "padding:4px calc(12 * var(--p-spacing-static-2xs));border-radius:calc(4px + (var(--p-leading-normal) / 2));";
      const blur = frosted[variant] ? "-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted);" : "";
      const iconRule = hasIcon ? "p-icon{margin-inline-start:-2px}" : "p-icon{display:none}";
      return ":not(:defined,[data-ssr]){visibility:hidden}span{position:relative;display:flex;gap:2px;" + pad + "font:var(--p-font-weight-normal) var(--p-typescale-xs)/var(--p-leading-normal) var(--p-font-porsche-next);" + blur + "color:" + text + ";background:" + bg + ';transition:color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out),background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out),backdrop-filter var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out)}::slotted(a),::slotted(button){all:unset!important;text-decoration:underline!important;cursor:pointer!important;font:inherit!important;color:inherit!important}::slotted(a)::before,::slotted(button)::before{content:""!important;position:absolute!important;inset:0!important;border-radius:var(--p-radius-full)!important}::slotted(a:focus-visible)::before,::slotted(button:focus-visible)::before{outline:2px solid var(--p-color-focus)!important;outline-offset:2px!important}::slotted(br){display:none!important}@media(forced-colors:active){span{outline:2px solid CanvasText;outline-offset:-2px;background-color:Canvas;color:CanvasText}::slotted(a:focus-visible)::before,::slotted(button:focus-visible)::before{outline-color:Highlight!important}}@media(hover:hover){span:hover{background:' + hover + "}}" + iconRule;
    }
    get iconName() {
      const icon = this.icon || "none";
...
`;
  __decorateClass([
    n4()
  ], LitTag.prototype, "variant", 2);
  __decorateClass([
    n4()
  ], LitTag.prototype, "compact", 2);
  __decorateClass([
    n4()
