...
`;
  ...
  render() {
    return html`<div popover="manual" inert=${this.isOpenFlag ? nothing : true} role=${this.roleName} aria-live=${this.ariaLive} aria-label=${this.headingAria || nothing}><style .innerHTML="${this.cssText}"></style><div class="notification">${this.headingNode}${this.descriptionNode}${this.dismissNode}</div></div>`;
  }
}
