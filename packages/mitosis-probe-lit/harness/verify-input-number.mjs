...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-number.html');
await page.waitForFunction(() => customElements.get('lit-input-number'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-number');
  return !!el?.shadowRoot?.querySelector('input[type="number"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-number'),
    hasShadowRoot: !!sr,
    ...
    cssTextHasSpin: !!style?.textContent?.includes('::-webkit-inner-spin-button'),
    ...
    minusTag: minus?.tagName ?? null,
    minusIsLit: minus?.tagName === 'LIT-BUTTON-PURE',
    minusIcon: minus?.getAttribute('icon'),
    minusText: minus?.textContent?.trim(),
    plusTag: plus?.tagName ?? null,
    plusIsLit: plus?.tagName === 'LIT-BUTTON-PURE',
    plusIcon: plus?.getAttribute('icon'),
    plusText: plus?.textContent?.trim(),
    buttonsHidden: buttons.some((btn) => getComputedStyle(btn).display === 'none'),
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-number');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-number');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-number');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-number');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-number');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-number');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const buttons = [...(el.shadowRoot?.querySelectorAll('p-button-pure') ?? [])];
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
    buttonsHidden: buttons.every((btn) => getComputedStyle(btn).display === 'none'),
  };
});

const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-number')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-number');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-number is not defined');
...
if (!live.cssTextHasSpin) failures.push('cssText missing webkit spin buttons');
...
if (live.inputType !== 'number') failures.push(`input type ${live.inputType}`);
if (live.inputValue !== '123') failures.push(`input value ${live.inputValue}`);
if (live.labelText !== 'Some label') failures.push(`label ${live.labelText}`);
if (live.buttonCount !== 2) failures.push(`button count ${live.buttonCount}`);
if (live.minusTag !== 'P-BUTTON-PURE') failures.push(`minus tag ${live.minusTag}`);
if (live.minusIsLit) failures.push('minus was swapped to lit-button-pure');
if (live.minusIcon !== 'minus') failures.push(`minus icon ${live.minusIcon}`);
if (live.minusText !== 'Decrement value by 1') failures.push(`minus text ${live.minusText}`);
if (live.plusTag !== 'P-BUTTON-PURE') failures.push(`plus tag ${live.plusTag}`);
if (live.plusIsLit) failures.push('plus was swapped to lit-button-pure');
if (live.plusIcon !== 'plus') failures.push(`plus icon ${live.plusIcon}`);
if (live.plusText !== 'Increment value by 1') failures.push(`plus text ${live.plusText}`);
if (live.buttonsHidden) failures.push('default controls hidden');
...
if (!hideLabel.buttonsHidden) failures.push('hide-label host still shows controls');
