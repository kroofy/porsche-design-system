...
const PLAYGROUND_URL = 'http://localhost:3333/?components=link';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_link_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_link_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/link_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_link_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-link.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-link'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="link"] p-link')];
  for (const el of hosts) {
    const lit = document.createElement('lit-link');
...
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="link"] lit-link')];
  return (
    hosts.length >= 7 &&
    hosts.every((el) => {
      ...
      if (!icon || getComputedStyle(icon).display === 'none') return true;
      const img = icon.shadowRoot?.querySelector('img');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="link"] lit-link')].filter((el) =>
      el.shadowRoot?.querySelector('.root'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="link"] lit-link')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLitIcon = await page.evaluate(
  () => !!document.querySelector('[data-card="link"] lit-link')?.shadowRoot?.querySelector('lit-icon'),
);
