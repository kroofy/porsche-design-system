...
@customElement("lit-tag-dismissible")
export default class LitTagDismissible extends LitElement {
  static styles = css`
      :host {
...
`;
  ...
  get cssText() {
    ...
    const labelRule = hasLabel
      ? ".label{display:block;margin-bottom:-4px;color:var(--p-color-contrast-high);font-size:var(--p-typescale-xs)}"
      : ".label{display:none}";
    return (
      ...
      ".sr-only,span.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}"
    );
  }
  get ariaLabel() {
