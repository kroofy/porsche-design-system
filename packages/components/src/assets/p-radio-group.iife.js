...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/radio-group/output/lit/RadioGroup.ts
  var LitRadioGroup = class extends i4 {
    get cssText() {
      const minWidth = {
        xs: 480,
        s: 760,
      ...
      const directionBase = typeof direction === "object" && direction !== null ? pick(direction, "base", "column") : direction;
      ...
      const dirFor = (d3) => d3 === "row" ? "flex-flow:row wrap;align-items:start" : "flex-flow:column nowrap;align-items:stretch";
      let out = ":host{--_p-radio-group-a:" + scale + ";--_p-radio-group-option-a:" + scale + '}:host([hidden]){display:none !important}slot[name="label-after"]{display:inline-block;vertical-align:top}slot[name="label-after"]::slotted(*){margin-inline-start:var(--p-spacing-static-xs) !important}.label-after{display:inline-block;vertical-align:top}';
      if (loading) {
        out += "::slotted(*:not([slot])){opacity:0.4 !important}@media(forced-colors:active){::slotted(*:not([slot])){opacity:1 !important;color:GrayText !important}}";
      }
      out += ":not(:defined,[data-ssr]){visibility:hidden}.root{all:unset;display:grid;justify-self:flex-start;row-gap:var(--p-spacing-static-xs)}.wrapper{position:relative;display:flex;" + dirFor(directionBase) + ";column-gap:calc(22.4px * (var(--_p-radio-group-a) - 0.64285714) + 8px);row-gap:calc(11.2px * (var(--_p-radio-group-a) - 0.64285714) + 4px)}";
      if (loading) {
      ...
      out += ".label{font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);cursor:" + (disabled ? "not-allowed" : "inherit") + ";color:var(--p-color-primary)";
      ...
      out += ".loading{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}";
      if (disabled) {
      ...
      if (typeof direction === "object" && direction !== null)
        for (const k2 of Object.keys(direction)) keys[k2] = 1;
      for (const bp of Object.keys(keys)) {
        if (bp === "base") continue;
        if (!minWidth[bp]) continue;
        let media = "@media(min-width:" + minWidth[bp] + "px){";
        if (typeof hideLabel === "object" && hideLabel !== null && hideLabel[bp] !== void 0) {
          const h3 = pick(hideLabel, bp, hideBase);
          media += ".label-wrapper{" + labelVisFor(h3) + "}.label:is(span){" + descVisFor(h3) + "}";
        }
        if (typeof direction === "object" && direction !== null && direction[bp] !== void 0) {
    ...
    get isLoading() {
      return (this.loading ?? this.getAttribute("loading")) === true || (this.loading ?? this.getAttribute("loading")) === "true" || (this.loading ?? this.getAttribute("loading")) === "";
    }
    get isRequired() {
    ...
    get labelNode() {
      ...
      const required = this.isRequired ? b2`<span class="required" aria-hidden="true"> *</span>` : A;
      const labelDisabled = this.isDisabled || this.isLoading;
      return b2`<div class="label-wrapper"><div class="label" id="label" aria-disabled=${labelDisabled ? "true" : A}>${this.labelText}<slot name="label"></slot>${required}</div><slot name="label-after"></slot></div>`;
    }
    get descriptionNode() {
      if (!this.hasDescription) return A;
      const labelDisabled = this.isDisabled || this.isLoading;
      return b2`<span class="label" id="description" aria-disabled=${labelDisabled ? "true" : A}>${this.descriptionText}<slot name="description"></slot></span>`;
    }
    get spinnerNode() {
      if (!this.isLoading) return A;
    ...
    stampOption(option) {
      ...
      const state2 = this.state ?? this.getAttribute("state") ?? "none";
      const name = this.name ?? this.getAttribute("name") ?? "";
      const optionValue = option.value ?? option.getAttribute("value");
      option.selected = optionValue === value && value !== null && value !== void 0;
    ...
    connectedCallback() {
      ...
      this.addEventListener("internalRadioGroupOptionChange", (e5) => {
        e5.stopPropagation();
        if (this.isDisabled || this.isLoading) return;
        const option = e5.target;
        this.value = option.value ?? option.getAttribute("value");
        this.syncOptions();
        this.requestUpdate();
      });
      this.addEventListener("internalRadioGroupOptionBlur", (e5) => {
        e5.stopPropagation();
      });
    }
    disconnectedCallback() {
    ...
    render() {
      return b2`<fieldset class="root" ?disabled=${!!this.isDisabled} role="radiogroup" aria-required=${this.isRequired ? "true" : A} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${this.hasLabel ? "label" : A} aria-describedby=${this.fieldsetDescribedBy()}><style .innerHTML="${this.cssText}"></style>${this.labelNode}${this.descriptionNode}<div class="wrapper"><slot></slot>${this.spinnerNode}</div><span class="message" id="message" role=${this.messageRole}>${this.iconNode}${this.messageText}</span><span class="loading" id="loading" role="status">${this.loadingText}</span></fieldset>`;
    }
  };
  LitRadioGroup.styles = i`
      :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "form", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "name", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "value", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "disabled", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "loading", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "compact", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "message", 2);
  __decorateClass([
    n4({ attribute: "hide-label" })
  ], LitRadioGroup.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "direction", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "label", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "description", 2);
  __decorateClass([
    n4()
  ], LitRadioGroup.prototype, "required", 2);
  LitRadioGroup = __decorateClass([
    t3("p-radio-group")
  ], LitRadioGroup);
})();
