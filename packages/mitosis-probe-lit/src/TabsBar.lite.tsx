...
useMetadata({ tagName: 'lit-tabs-bar' });
  ...
  const state = useStore({
    get cssText(): string {
      ...
      if (rawIndex === undefined || rawIndex === null || rawIndex === '') {
        ...
        if (!Number.isInteger(n) || tabCount < 1 || n < 0 || n > tabCount - 1) active = undefined;
        else active = n;
      }
      ...
      if (hasBackground) {
        out +=
          ...
          bgMap[background] +
          ';padding:' +
          (compact ? 'calc(3 * var(--p-spacing-static-2xs))' : 'var(--p-spacing-static-xs)') +
          ';border-radius:' +
