...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitPinCode = class extends i4 {
    ...
    render() {
      ...
      const inputs = Array.from({ length }, (_2, index) => {
        const ch = value[index];
        const val = !ch || ch === " " ? "" : ch;
        const isCurrent = this.isCurrentInput(index, value, length);
        return b2`<input id=${isCurrent ? "current-input" : A} type=${type} aria-label=${index + 1 + "-" + length} aria-describedby=${this.inputDescribedBy()} aria-invalid=${this.ariaInvalid || A} aria-disabled=${loading ? "true" : A} autocomplete="one-time-code" pattern="\\d*" inputmode="numeric" .value=${val} ?disabled=${disabled} ?required=${required} name=${this.name || A} form=${this.form || A}>`;
      });
      const spinner = loading ? b2`<p-spinner class="spinner" size="inherit" aria-hidden="true"></p-spinner>` : A;
      return b2`<fieldset class="root" ?disabled=${disabled} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${this.labelText ? "label" : A} aria-describedby=${this.fieldsetDescribedBy()}><style .innerHTML="${this.cssText}"></style><div class="label-wrapper"><label class="label" id="label" for="current-input" aria-disabled=${disabled || loading ? "true" : A}>${this.labelText}</label><slot name="label-after"></slot></div><span class="label" id="description">${this.descriptionText}</span><div class="wrapper" dir="ltr">${inputs}${spinner}</div><span class="message" id="message" role=${this.messageRole}><p-icon name=${this.iconName || A} color=${this.iconColor || A} aria-hidden="true"></p-icon>${this.messageText}</span><span class="loading" id="loading" role="status">${this.loadingText}</span></fieldset>`;
    }
  };
  LitPinCode.styles = i`
      :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "form", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "name", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "disabled", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "loading", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "compact", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "message", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "length", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "label", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "description", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "required", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "type", 2);
  __decorateClass([
    n4()
  ], LitPinCode.prototype, "value", 2);
  LitPinCode = __decorateClass([
    t3("lit-pin-code")
  ], LitPinCode);
})();
