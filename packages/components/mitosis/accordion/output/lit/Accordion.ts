...
`;
  ...
  render() {
    return html`<details ?open=${!!this.isOpenFlag}><style .innerHTML="${this.cssText}"></style><summary @click=${this.onSummaryClick}>${this.summaryNode}</summary>${this.beforeNode}${this.afterNode}<div><slot></slot></div></details>`;
  }
}
