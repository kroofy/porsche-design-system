...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-date.html');
await page.waitForFunction(() => customElements.get('lit-input-date'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-date');
  return !!el?.shadowRoot?.querySelector('input[type="date"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-date'),
    hasShadowRoot: !!sr,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-date');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-date');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-date');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-date');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-date');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-date');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-date')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-date');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-date is not defined');
...
if (live.inputType !== 'date') failures.push(`input type ${live.inputType}`);
if (live.inputValue !== '1931-04-25') failures.push(`input value ${live.inputValue}`);
