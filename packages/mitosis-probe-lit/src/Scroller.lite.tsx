...
useMetadata({ tagName: 'lit-scroller' });
...
export default function LitScroller(props: {
  ...
  const state = useStore({
    get cssText(): string {
      ...
      let hasBar: any = props.scrollbar;
      if (hasBar === true || hasBar === 'true' || hasBar === '') {
        hasBar = true;
      } else {
        hasBar = false;
      }
      let isCompact: any = props.compact;
      if (isCompact === true || isCompact === 'true' || isCompact === '') {
        isCompact = true;
      } else {
        isCompact = false;
      }
      let isSticky: any = props.sticky;
      ...
      for (let i = 1; i < steps; i++) {
        ...
        rightStops.push(
          'rgb(0 0 0/' + (1 - alpha).toFixed(3) + ') calc(100% - ' + (fullLength - fadeLength * t).toFixed(0) + 'px)',
        );
      }
      const left =
        'transparent 0px,transparent ' + edgeLength + 'px,' + leftStops.join(',') + ',black ' + fullLength + 'px';
      const right =
        'black calc(100% - ' +
        fullLength +
        'px),' +
        ...
        'px),transparent 100%';
      let fade = '';
      if (fadeEdges === 'left') fade = 'linear-gradient(to right,' + left + ',black 100%)';
      else if (fadeEdges === 'right') fade = 'linear-gradient(to right,black 0%,' + right + ')';
      else if (fadeEdges === 'both') fade = 'linear-gradient(to right,' + left + ',' + right + ')';
      ...
      if (hasBar) {
        ...
        if (maskLayer) out += '.scroll{-webkit-mask:' + maskLayer + ';mask:' + maskLayer + '}';
