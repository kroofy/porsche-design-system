...
const BASELINE_PNG =
  process.env.BASELINE_PNG ?? '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_pagination_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_pagination_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/pagination_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_pagination_control.png`;
const VIEWPORT = { width: 1440, height: 900 };
const DEVICE_SCALE_FACTOR = 2;

const hostReady = (tag) => {
  const hosts = [...document.querySelectorAll(`[data-card="pagination"] ${tag}`)];
  return (
    hosts.length >= 2 &&
    hosts.every((el) => {
      if (tag === 'p-pagination' && !el.classList.contains('hydrated')) return false;
      const nav = el.shadowRoot?.querySelector('nav');
      const icons = [...(el.shadowRoot?.querySelectorAll('p-icon') ?? [])];
      if (!nav || icons.length < 2) return false;
      return icons.every((icon) => {
        if (icon.tagName !== 'P-ICON') return false;
        if (!icon.classList.contains('hydrated')) return false;
...
await page.waitForFunction(hostReady, 'p-pagination', { timeout: 30_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-pagination.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-pagination');
...
await page.waitForFunction(hostReady, 'lit-pagination', { timeout: 30_000 });
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="pagination"] lit-pagination')].filter((el) =>
      el.shadowRoot?.querySelector('nav'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="pagination"] lit-pagination')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document.querySelector('[data-card="pagination"] lit-pagination')?.shadowRoot?.querySelector('lit-icon,lit-button'),
);
swap.icons = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="pagination"] lit-pagination')].flatMap((el) =>
      [...(el.shadowRoot?.querySelectorAll('p-icon') ?? [])].map((icon) => icon.tagName),
    ),
);
...
const failed =
  ...
  swap.innerLit ||
  swap.swapped < 2 ||
  (swap.icons && swap.icons.some((tag) => tag !== 'P-ICON')) ||
