...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-url.html');
await page.waitForFunction(() => customElements.get('lit-input-url'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-url');
  return !!el?.shadowRoot?.querySelector('input[type="url"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-url'),
    hasShadowRoot: !!sr,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-url');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-url');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-url');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-url');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-url');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-url');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-url')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-url');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-url is not defined');
...
if (live.inputType !== 'url') failures.push(`input type ${live.inputType}`);
...
if (live.indicatorName !== 'linked') failures.push(`indicator name ${live.indicatorName}`);
