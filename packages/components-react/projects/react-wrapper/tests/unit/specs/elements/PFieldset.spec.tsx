...
describe('PFieldset', () => {
  it('returns a fieldset with nothing wrapping it', () => {
    ...
    expect(container.innerHTML).toBe('<fieldset class="p-fieldset"><legend>Some label</legend></fieldset>');
  });

  it('puts aria attributes on the fieldset', () => {
    const { container } = render(<PFieldset role="radiogroup" aria-required="true" />);
  ...
  it('encodes non-default appearance on the fieldset', () => {
    const { container } = render(<PFieldset labelSize="small" required state="error" />);
    const fieldset = container.firstElementChild as HTMLFieldSetElement;
