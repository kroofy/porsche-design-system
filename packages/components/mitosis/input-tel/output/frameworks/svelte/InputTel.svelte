<!-- mitosis-native-host: native svelte from InputTel.lite.tsx -->
...
<script lang="ts">
  ...
  export let disabled: LitInputTelProps["disabled"];
  export let loading: LitInputTelProps["loading"];
  export let compact: LitInputTelProps["compact"];
  export let readOnly: LitInputTelProps["readOnly"];
  export let indicator: LitInputTelProps["indicator"];
  export let state: LitInputTelProps["state"];
  export let message: LitInputTelProps["message"];
  export let hideLabel: LitInputTelProps["hideLabel"];
  export let label: LitInputTelProps["label"];
  export let description: LitInputTelProps["description"];
  export let value: LitInputTelProps["value"];
  export let maxLength: LitInputTelProps["maxLength"];
  export let placeholder: LitInputTelProps["placeholder"];
  function __cmpProps() { return { disabled, loading, compact, readOnly, indicator, state, message, hideLabel, label, description, value, maxLength, placeholder }; }
  ...
  $: scopedCssText = scopeCss("\n  :host([hidden]) {\n    display: none !important;\n  }\n" + (typeof cssText === "function" ? cssText() : (cssText || "")), ".p-input-tel");
</script>

<div class="p-input-tel" data-pds="input-tel">
<div class="root">
  {@html `<${"style"}>${scopedCssText}<${"/style"}>`}
  <div class="label-wrapper">
    <label class="label" id="label" for="input-tel">{labelText()}</label><slot
      name="label-after"
    />
  </div>
  ...
  <div class="wrapper">
    <slot name="start" /><PIcon
      ...
      color="contrast-low"
      aria-hidden="true"
    /><input type="tel" id="input-tel" dir="auto"  value={inputValue()} placeholder={placeholderText()} disabled={!!isDisabled()} readonly={!!isReadOnly()} maxlength={maxLengthValue()} /><slot
      name="end"
