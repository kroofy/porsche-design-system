...
const hostReady = () => {
  const hosts = [
    ...document.querySelectorAll('[data-card="input-date"] p-input-date, [data-card="input-date"] lit-input-date'),
  ];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      const sr = el.shadowRoot;
      if (!sr?.querySelector('input[type="date"]')) return false;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-input-date.bundle.js` });
...
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-input-date');
...
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="input-date"] lit-input-date')].filter((el) =>
      el.shadowRoot?.querySelector('input[type="date"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="input-date"] lit-input-date')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="input-date"] lit-input-date')
