...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitInputText = class extends i4 {
    ...
    get hasCounter() {
      const raw = this.counter ?? this.getAttribute("counter");
    ...
    get isReadOnly() {
      const raw = this.readOnly ?? this.getAttribute("read-only");
      return raw === true || raw === "true" || raw === "";
    }
    get ariaDisabled() {
    ...
    get ariaReadonly() {
      const rawRead = this.readOnly ?? this.getAttribute("read-only");
      return rawRead === true || rawRead === "true" || rawRead === "" ? "true" : "";
    }
    get loadingText() {
      const loading = this.loading === true || this.loading === "true" || this.loading === "";
