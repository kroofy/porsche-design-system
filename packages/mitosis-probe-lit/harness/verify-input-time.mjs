...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/input-time.html');
await page.waitForFunction(() => customElements.get('lit-input-time'));
await page.waitForFunction(() => {
  const el = document.querySelector('#default lit-input-time');
  return !!el?.shadowRoot?.querySelector('input[type="time"]');
});
...
const live = await page.evaluate(() => {
  ...
  return {
    isDefined: !!customElements.get('lit-input-time'),
    hasShadowRoot: !!sr,
...
const loading = await page.evaluate(() => {
  const el = document.querySelector('#loading lit-input-time');
...
const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-input-time');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-input-time');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-input-time');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
...
const readonly = await page.evaluate(() => {
  const el = document.querySelector('#readonly lit-input-time');
  const wrap = el.shadowRoot?.querySelector('.wrapper');
...
const hideLabel = await page.evaluate(() => {
  const el = document.querySelector('#hide-label lit-input-time');
...
const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-input-time')).display);
...
const at999 = await page.evaluate(() => {
  const el = document.querySelector('#breakpoint lit-input-time');
  const wrap = el.shadowRoot?.querySelector('.label-wrapper');
  const style = el.shadowRoot?.querySelector('style');
  return {
    overflow: wrap ? getComputedStyle(wrap).overflow : null,
...
const failures = [];
if (!live.isDefined) failures.push('lit-input-time is not defined');
...
if (live.cssTextHasMoz) failures.push('cssText should not have moz-appearance');
...
if (live.inputType !== 'time') failures.push(`input type ${live.inputType}`);
if (live.inputValue !== '09:11') failures.push(`input value ${live.inputValue}`);
...
if (live.calendarIcon !== 'clock') failures.push(`clock icon ${live.calendarIcon}`);
if (live.calendarText !== 'Open time picker') failures.push(`calendar text ${live.calendarText}`);
