...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/drilldown-item/output/lit/DrilldownItem.ts
  var LitDrilldownItem = class extends i4 {
    constructor() {
      super(...arguments);
      this._onCascadeClick = () => {
        const parent = this.parentElement;
        const isDrilldownParent = parent && parent.tagName === "P-DRILLDOWN";
        if (isDrilldownParent) {
          this._emitInternalUpdate(this.isSecondaryFlag ? void 0 : this.identifierValue);
    ...
    get cssText() {
      ...
      let h22 = "h2{display:none}";
      let slottedExtra = "";
      if (isCascade)
        slottedExtra += "::slotted(*:not([primary],[cascade])){display:none !important}";
      if (isPrimary)
        slottedExtra += "@media(max-width:759px){::slotted(*:not([secondary])){display:none}}";
      ...
      if (isPrimary || isCascade) button += ".button{display:none}";
        ...
        button += ".button{grid-column:1/-1;padding:var(--p-spacing-fluid-sm);margin:0 calc(var(--p-spacing-fluid-sm) * -1)}";
      let back = "";
      if (!isPrimary) back = ".back{display:none}";
      let mobile = "";
      let desktop = "";
      if (isSecondary) {
        headerSlot += '@media(max-width:759px){slot[name="header"]{grid-area:2/3;display:grid;place-items:center;z-index:2}}';
        buttonSlot += '@media(max-width:759px){slot[name="button"]{display:none}}';
        defaultSlot += "@media(max-width:759px){slot:not([name]){grid-area:4/2/auto/-2;z-index:0;display:grid;" + slotGrid + ";animation:slide-up-mobile " + animMd + " " + easeBase + "}}";
        h22 += "@media(max-width:759px){h2{font:var(--p-font-weight-semibold) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);display:block;grid-area:2/3;place-self:center;z-index:2;margin:0;padding-inline:var(--p-spacing-static-md);max-width:100%;box-sizing:border-box;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--_p-drilldown-a)}}";
        scroller += '@media(max-width:759px){.scroller{display:grid;grid-template-rows:subgrid;grid-template-columns:subgrid;grid-area:1/1/-1/-1}.scroller::before{z-index:1;content:"";position:sticky;top:0;grid-area:1/1/4/-1;background:linear-gradient(180deg,var(--_p-drilldown-b) 0%,var(--_p-drilldown-b) 65%,transparent 100%)}}@media(min-width:760px){.scroller{grid-area:1/1/-1/-1;display:grid;grid-template-rows:subgrid;grid-template-columns:subgrid}}';
        button += "@media(max-width:759px){.button{display:none}}";
        mobile += ".drawer{display:contents}";
        desktop += ".drawer{position:absolute;inset:0;inset-inline-start:clamp(338px, 210px + 18vw, 640px);display:grid;grid-template:var(--p-spacing-fluid-md) minmax(0, 1fr)/var(--p-spacing-fluid-lg) minmax(0, 1fr) var(--p-spacing-fluid-lg)}";
      }
      if (isPrimary || isCascade) {
        defaultSlot += "@media(max-width:759px){slot:not([name]){display:contents}}";
        scroller += "@media(max-width:759px){.scroller{display:contents}}@media(min-width:760px){.scroller{display:contents}}";
        mobile += ".drawer{display:contents}";
        desktop += ".drawer{display:contents}";
      }
      if (isPrimary || isSecondary) {
        defaultSlot += "@media(min-width:760px){slot:not([name]){grid-area:3/2/auto/-2;display:grid;" + slotGrid + ";animation:slide-up-desktop-" + (isPrimary ? "primary" : "secondary") + " " + animMd + " " + easeBase + "}}";
      }
      if (isSecondary) {
        defaultSlot += "@media(min-width:760px){slot:not([name]){grid-area:2/2/auto/-2;padding-block-end:var(--p-spacing-fluid-lg)}}";
      }
      if (isCascade) {
      ...
      if (isPrimary) {
        back += "@media(max-width:759px){.back{grid-area:2/2;margin-top:2px;width:fit-content;height:fit-content;place-self:start;z-index:2}}@media(min-width:760px){.back{grid-area:2/2;margin-bottom:var(--p-spacing-fluid-md);width:fit-content;height:fit-content;margin-inline-start:-4px}}";
      }
      return "@keyframes slide-up-mobile{from{transform:translate3d(0,var(--p-spacing-fluid-md),0)}to{transform:translate3d(0,0,0)}}@keyframes slide-up-desktop-primary{from{margin-block-start:var(--p-spacing-fluid-md)}to{margin-block-start:0px}}@keyframes slide-up-desktop-secondary{from{margin-block-start:var(--p-spacing-fluid-md)}to{margin-block-start:0px}}:host{display:contents}:host([hidden]){display:none !important}:not(:defined,[data-ssr]){visibility:hidden}" + headerSlot + buttonSlot + defaultSlot + h22 + slottedExtra + "::slotted(*){--p-drilldown-grid-template:auto/auto;--p-drilldown-gap:var(--p-spacing-fluid-xs)}" + scroller + button + back + "@media(min-width:760px){" + desktop + "}@media(max-width:759px){" + mobile + "}";
    }
    get labelValue() {
    ...
    render() {
      ...
      const cascade = hasButton ? b2`<slot name="button"></slot>` : b2`<p-button-pure class="button" type="button" size="medium" align-label="start" stretch="true" icon="arrow-head-right" icon-source="http://localhost:3001/icons/arrow-head-right.304b330.svg" ?inert=${isPrimary || isCascade} active=${isSecondary ? "true" : A} aria-expanded=${isSecondary ? "true" : "false"} @click=${this._onCascadeClick}>${label}</p-button-pure>`;
      const header = hasHeader ? b2`<slot name="header"></slot>` : b2`<h2>${label}</h2>`;
      return b2`<style .innerHTML="${this.cssText}"></style>${cascade}<p-button-pure class="back" type="button" size="small" align-label="end" stretch="true" icon="arrow-left" icon-source="http://localhost:3001/icons/arrow-left.e03c25b.svg" hide-label='{"base":true,"s":false}' @click=${this._onBackClick}>${label}</p-button-pure>${header}<div class="drawer"><div class="scroller"><slot></slot></div></div>`;
    }
  };
  LitDrilldownItem.styles = i`
      :host {
          display: contents;
        }
        :host([hidden]) {
...
`;
  __decorateClass([
    n4()
  ], LitDrilldownItem.prototype, "identifier", 2);
  __decorateClass([
    n4()
  ], LitDrilldownItem.prototype, "primary", 2);
  __decorateClass([
    n4()
  ], LitDrilldownItem.prototype, "secondary", 2);
  __decorateClass([
    n4()
  ], LitDrilldownItem.prototype, "cascade", 2);
  __decorateClass([
    n4()
  ], LitDrilldownItem.prototype, "label", 2);
  LitDrilldownItem = __decorateClass([
    t3("p-drilldown-item")
  ], LitDrilldownItem);
})();
