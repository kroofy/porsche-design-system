...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/button-pure/output/lit/ButtonPure.ts
  var LitButtonPure = class extends i4 {
    get cssText() {
      ...
      if (hasIcon) {
        out += ".icon,p-icon,p-spinner{position:relative;flex-shrink:0}.label{z-index:1;" + visFor(hideBase) + ";order:" + orderFor(alignBase) + "}";
      } else if (loading) {
        out += ".label{position:relative;opacity:0}.icon,p-spinner{position:absolute;top:0;left:calc(50% - var(--p-leading-normal) / 2)}";
      } else {
    ...
    render() {
      return b2`
          ...
          <p-icon class="icon"
            ...
            .source="${this.iconSrc}"
          ></p-icon>
          <p-spinner class="icon" size="inherit" color="inherit" aria-hidden="true"></p-spinner>
