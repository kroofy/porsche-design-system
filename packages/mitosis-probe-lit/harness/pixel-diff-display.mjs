...
const PLAYGROUND_URL = 'http://localhost:3333/?components=display';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_display_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_display_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/display_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_display_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-display.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-display'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="display"] p-display')];
  for (const el of hosts) {
    const lit = document.createElement('lit-display');
...
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="display"] lit-display')];
  return (
    hosts.length >= 4 &&
    hosts.every((el) => {
      const slot = el.shadowRoot?.querySelector('slot');
      return !!el.shadowRoot?.querySelector('h3') && (slot?.assignedNodes({ flatten: true }).length ?? 0) > 0;
    })
  );
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="display"] lit-display')].filter((el) =>
      el.shadowRoot?.querySelector('h3')
    ).length
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="display"] lit-display')?.shadowRoot?.querySelector('my-fragment')
);
