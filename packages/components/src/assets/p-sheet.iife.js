...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/sheet/output/lit/Sheet.ts
  var LitSheet = class extends i4 {
    get cssText() {
      ...
      const dialogVis = isOpen ? "width:100dvw;height:100dvh;visibility:inherit;pointer-events:auto;background:var(--p-color-backdrop)" : "width:0px;height:0px;visibility:hidden;pointer-events:none;background:transparent";
      ...
      let out = ":host{display:contents;--ref-p-sheet-pt:var(--p-spacing-fluid-md) !important;--ref-p-sheet-pb:calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md)) !important;--ref-p-sheet-px:var(--p-spacing-fluid-lg) !important;--pds-internal-grid-outer-column:calc(var(--p-spacing-fluid-lg) - clamp(16px, 1.25vw + 12px, 36px)) !important;--pds-internal-grid-margin:calc(var(--p-spacing-fluid-lg) * -1) !important;--pds-internal-grid-width-min:auto !important;--pds-internal-grid-width-max:none !important;--_p-dialog-a:" + dialogBg + " !important}:host([hidden]){display:none !important}slot{display:block}slot:first-of-type{grid-row-start:1}slot:not([name]){grid-column:2/3;z-index:0}slot[name=header]{grid-column:2/3;z-index:0}dialog{all:unset;position:fixed;inset:0;max-width:100dvw;max-height:100dvh;overflow:hidden;display:block;user-select:text;outline:0;" + dialogVis + ";transition:" + dialogTrans + ";overlay:none}dialog:modal{overlay:auto}dialog::backdrop{display:none}@supports (overlay: auto) and (transition-behavior: allow-discrete){dialog{transition:" + dialogTrans + ", overlay " + dialogDur + " " + ease + " allow-discrete}}.scroller{position:absolute;isolation:isolate;display:grid;inset:0;overflow:hidden auto;overscroll-behavior-y:none;background:rgba(255,255,255,.01);transform:translate3d(0,0,0)}.sheet{position:relative;display:grid;grid-template:auto/var(--p-spacing-fluid-sm) minmax(0,1fr) var(--p-spacing-fluid-sm);gap:var(--p-spacing-fluid-md) calc(var(--p-spacing-fluid-lg) - var(--p-spacing-fluid-sm));padding-top:var(--p-spacing-fluid-md);padding-bottom:calc(var(--p-radius-3xl) + var(--p-spacing-fluid-md));align-content:flex-start;" + panelClosed + ";transition:opacity " + panelDur + " " + ease + ", transform " + panelDur + " " + ease + ";color:var(--p-color-primary);background:var(--_p-dialog-a);width:100%;align-self:flex-end;margin-block-start:var(--p-spacing-fluid-lg);border-top-left-radius:var(--p-radius-3xl);border-top-right-radius:var(--p-radius-3xl);clip-path:inset(0 round var(--p-radius-3xl) var(--p-radius-3xl) 0 0)}" + (isOpen ? "" : ".sheet:dir(rtl){transform:translate3d(0,25vh,0)}") + "@media(forced-colors:active){.sheet{border-top:2px solid CanvasText}}";
      if (dismiss) {
    ...
    render() {
      const dismiss = this.showDismiss ? b2`<button class="dismiss" type="button"><span>Dismiss sheet</span></button>` : A;
      const label = this.ariaLabelText || A;
      return b2`<style .innerHTML="${this.cssText}"></style><dialog ?inert=${!this.isOpenFlag} tabindex="-1" aria-modal="true" aria-label=${label}><div class="scroller"><div class="sheet">${dismiss}<slot name="header"></slot><slot></slot></div></div></dialog>`;
    }
  };
  LitSheet.styles = i`
      :host {
...
`;
  __decorateClass([
    n4()
  ], LitSheet.prototype, "open", 2);
  __decorateClass([
    n4()
  ], LitSheet.prototype, "dismissButton", 2);
  __decorateClass([
    n4()
  ], LitSheet.prototype, "background", 2);
  __decorateClass([
    n4()
  ], LitSheet.prototype, "aria", 2);
  LitSheet = __decorateClass([
    t3("p-sheet")
  ], LitSheet);
})();
