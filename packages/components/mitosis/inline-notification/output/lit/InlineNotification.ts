...
@customElement("p-inline-notification")
...
`;
  ...
  render() {
    return html`<div class="notification" role=${this.roleName} aria-live=${this.ariaLive} aria-label=${this.headingAria || nothing}><style .innerHTML="${this.cssText}"></style>${this.headingNode}${this.descriptionNode}${this.actionNode}${this.dismissNode}</div>`;
  }
}
