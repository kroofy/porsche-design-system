...
const BASELINE_PNG =
  process.env.BASELINE_PNG ?? '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_select_before.png';
const AFTER_PNG = process.env.AFTER_PNG ?? '/opt/cursor/artifacts/mitosis_lit_select_after.png';
const DIFF_PNG = process.env.DIFF_PNG ?? '/opt/cursor/artifacts/select_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_select_control.png`;
...
const hostReady = (tag) => {
  const hosts = [...document.querySelectorAll(`[data-card="select"] ${tag}`)];
  return (
    hosts.length >= 4 &&
    hosts.every((el) => {
      if (tag === 'p-select' && !el.classList.contains('hydrated')) return false;
      ...
      const options = [...el.querySelectorAll(':scope > p-select-option, :scope > p-optgroup')];
...
await page.waitForFunction(hostReady, 'p-select', { timeout: 30_000 });
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-select.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-select'));
const swap = await page.evaluate(() => {
  ...
  for (const el of hosts) {
    const lit = document.createElement('lit-select');
    ...
    for (const child of [...el.querySelectorAll(':scope > p-select-option, :scope > p-optgroup')]) {
...
await pauseCardAnimation(page);
await page.waitForFunction(hostReady, 'lit-select', { timeout: 30_000 });
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="select"] lit-select')].filter((el) =>
      el.shadowRoot?.querySelector('button[role="combobox"], button'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="select"] lit-select')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () =>
    !!document
      .querySelector('[data-card="select"] lit-select')
      ?.shadowRoot?.querySelector('lit-icon,lit-select-option,lit-optgroup') ||
    !!document.querySelector('[data-card="select"] lit-select-option, [data-card="select"] lit-optgroup'),
);
swap.slottedCopied = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="select"] lit-select')].every(
      (el) => el.querySelectorAll(':scope > p-select-option, :scope > p-optgroup').length >= 3,
    ),
);
swap.closed = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="select"] lit-select')].every((el) => {
...
swap.itemsStayStencil = await page.evaluate(() => {
  const options = [...document.querySelectorAll('[data-card="select"] lit-select p-select-option')];
  const groups = [...document.querySelectorAll('[data-card="select"] lit-select > p-optgroup')];
  const icons = [...document.querySelectorAll('[data-card="select"] lit-select')].flatMap((host) => [
...
const failed =
  ...
  !swap.itemsStayStencil?.tags.includes('P-SELECT-OPTION') ||
