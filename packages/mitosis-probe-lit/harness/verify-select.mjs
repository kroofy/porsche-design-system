...
await page.goto('file:///workspace/packages/mitosis-probe-lit/harness/select.html');
await page.waitForFunction(() => customElements.get('lit-select'));
await page.waitForFunction(() => document.querySelector('#default lit-select')?.shadowRoot?.querySelector('button'));

const live = await page.evaluate(() => {
  const el = document.querySelector('#default lit-select');
  ...
  return {
    isDefined: !!customElements.get('lit-select'),
    hasShadowRoot: !!sr,
    dynamicStyleTag: !!style,
    cssHasRoot: css.includes('.root{display:grid'),
    cssHasScale: css.includes('--_p-select-a:1') && css.includes('--_p-select-option-a:1') && css.includes('--_p-optgroup-a:1'),
    ...
    innerLit: !!sr?.querySelector('lit-icon,lit-select-option,lit-optgroup'),
    slotted: slot ? slot.assignedElements().map((n) => n.getAttribute('data-item')) : [],
  };
});

const disabled = await page.evaluate(() => {
  const el = document.querySelector('#disabled lit-select');
...
const success = await page.evaluate(() => {
  const el = document.querySelector('#success lit-select');
...
const errorState = await page.evaluate(() => {
  const el = document.querySelector('#error lit-select');
  const icon = el.shadowRoot?.querySelector('.message p-icon');
  const button = el.shadowRoot?.querySelector('button');
  const css = el.shadowRoot?.querySelector('style')?.textContent ?? '';
  return {
...
const compact = await page.evaluate(() => {
  const css = document.querySelector('#compact lit-select')?.shadowRoot?.querySelector('style')?.textContent ?? '';
  return {
    cssHasCompactScale: css.includes('--_p-select-a:0.64285714'),
    cssHasCompactRadius: css.includes('var(--p-radius-lg)'),
  };
});

const hideLabel = await page.evaluate(() => {
  const css = document.querySelector('#hide-label lit-select')?.shadowRoot?.querySelector('style')?.textContent ?? '';
  return {
    cssHasMedia: css.includes('@media(min-width:1000px){.label-wrapper{position:absolute'),
  };
});

const hidden = await page.evaluate(() => getComputedStyle(document.querySelector('#hidden lit-select')).display);

await browser.close();

const failures = [];
if (!live.isDefined) failures.push('lit-select is not defined');
