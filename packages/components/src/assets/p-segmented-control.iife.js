...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // ../../components/mitosis/segmented-control/output/lit/SegmentedControl.ts
  ...
  var colWidthFor = (col, measuredMin, measuredMax) => {
    if (col === "auto" || col === void 0 || col === null || col === "") {
      const w2 = measuredMax > 220 && 220 || measuredMax < 46 && measuredMin || measuredMax;
      return "repeat(auto-fit, " + w2 + "px)";
    }
    return "repeat(" + col + ", minmax(0, 1fr))";
  };
  var assignHide = (vars, bp, hidden) => {
    const p3 = bp === "base" ? "--p-sc-lw" : `--p-sc-lw-${bp}`;
    const d3 = bp === "base" ? "--p-sc-desc" : `--p-sc-desc-${bp}`;
    if (hidden) {
  ...
  var LitSegmentedControl = class extends i4 {
    get hostStyle() {
      ...
      const formState = (this.getAttribute("state") ?? this.state) === "success" || (this.getAttribute("state") ?? this.state) === "error" ? this.state : "none";
      ...
      const measuredMin = measured.minWidth;
      const measuredMax = measured.maxWidth;
      const palettes = {
        none: "",
      ...
      if (noWrap) {
        ...
        let last = colWidthFor(
          pick(columns, "base", "auto"),
          measuredMin,
          measuredMax
        );
        for (const bp of BREAKPOINTS) {
          if (columns[bp] !== void 0)
            last = colWidthFor(
              pick(columns, bp, "auto"),
              measuredMin,
              measuredMax
            );
    ...
    get labelNode() {
      ...
      const after = this.hasLabelAfter ? b2`<slot name="label-after"></slot>` : A;
      return b2`<div class="label-wrapper"><div class="label" id="label" aria-disabled=${this.isDisabled ? "true" : A}>${this.labelText}<slot name="label"></slot>${required}</div>${after}</div>`;
    }
    get descriptionNode() {
    ...
    get slotNode() {
      if (this.isNoWrap) return b2`<p-scroller class="scroller"><slot></slot></p-scroller>`;
      return b2`<slot></slot>`;
    }
    get iconNode() {
    ...
    measureItemWidths(compact) {
      ...
      const dimension = "calc(max(var(--p-leading-normal), " + scaling + " * (var(--p-leading-normal) + 10px)) + (" + verticalPadding + " + 1px) * 2)";
    ...
    stampItem(item) {
      ...
      item.selected = value !== null && value !== void 0 && String(itemValue) === String(value);
      item.state = state2;
    ...
    connectedCallback() {
      ...
      this.addEventListener("internalSegmentedControlItemUpdate", (e5) => {
        e5.stopPropagation();
        if (this.isDisabled) return;
        const item = e5.target;
    ...
    render() {
      return b2`<fieldset class="root" ?disabled=${!!this.isDisabled} aria-invalid=${this.ariaInvalid || A} aria-labelledby=${this.hasLabel ? "label" : A} aria-describedby=${this.hasDescription ? "description" : A}>${this.labelNode}${this.descriptionNode}${this.slotNode}<span class="message" id="message" role=${this.messageRole}>${this.iconNode}${this.messageText}</span></fieldset>`;
    }
  };
  LitSegmentedControl.styles = i`
      :host([hidden]) {
        ...
        @media (min-width: 760px) {
          slot:not([name]) {
            grid-template-columns: var(
              --p-sc-cols-s,
              var(--p-sc-cols-xs, var(--p-sc-cols))
            );
          }
          .label-wrapper {
            min-width: var(
              --p-sc-lw-s-minw,
              var(--p-sc-lw-xs-minw, var(--p-sc-lw-minw))
            );
            position: var(
              --p-sc-lw-s-pos,
              var(--p-sc-lw-xs-pos, var(--p-sc-lw-pos, static))
            );
            ...
            overflow: var(
              --p-sc-lw-s-ov,
              var(--p-sc-lw-xs-ov, var(--p-sc-lw-ov, visible))
            );
            clip: var(
              --p-sc-lw-s-clip,
              var(--p-sc-lw-xs-clip, var(--p-sc-lw-clip, auto))
            );
            white-space: var(
              --p-sc-lw-s-ws,
              var(--p-sc-lw-xs-ws, var(--p-sc-lw-ws, normal))
            );
          }
          .label:is(span) {
            position: var(
              --p-sc-desc-s-pos,
              var(--p-sc-desc-xs-pos, var(--p-sc-desc-pos, static))
            );
            width: var(
              --p-sc-desc-s-w,
              var(--p-sc-desc-xs-w, var(--p-sc-desc-w, auto))
            );
            height: var(
              --p-sc-desc-s-h,
              var(--p-sc-desc-xs-h, var(--p-sc-desc-h, auto))
            );
            padding: var(
              --p-sc-desc-s-pad,
              var(--p-sc-desc-xs-pad, var(--p-sc-desc-pad, 0))
            );
            margin: var(--p-sc-desc-s-m, var(--p-sc-desc-xs-m, var(--p-sc-desc-m, 0)));
            overflow: var(
              --p-sc-desc-s-ov,
              var(--p-sc-desc-xs-ov, var(--p-sc-desc-ov, visible))
            );
            clip: var(
              --p-sc-desc-s-clip,
              var(--p-sc-desc-xs-clip, var(--p-sc-desc-clip, auto))
            );
            white-space: var(
              --p-sc-desc-s-ws,
              var(--p-sc-desc-xs-ws, var(--p-sc-desc-ws, normal))
            );
            margin-top: calc(-1 * var(--p-spacing-static-xs));
          }
        }
        @media (min-width: 1000px) {
          slot:not([name]) {
            grid-template-columns: var(
              --p-sc-cols-m,
              var(--p-sc-cols-s, var(--p-sc-cols-xs, var(--p-sc-cols)))
            );
          }
          .label-wrapper {
...
`;
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "disabled", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "compact", 2);
  __decorateClass([
    n4({ attribute: "no-wrap" })
  ], LitSegmentedControl.prototype, "noWrap", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "state", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "message", 2);
  __decorateClass([
    n4({ attribute: "hide-label" })
  ], LitSegmentedControl.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "columns", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "label", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "description", 2);
  __decorateClass([
    n4()
  ], LitSegmentedControl.prototype, "required", 2);
  LitSegmentedControl = __decorateClass([
    t3("p-segmented-control")
  ], LitSegmentedControl);
})();
