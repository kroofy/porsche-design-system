...
const PLAYGROUND_URL = 'http://localhost:3333/?components=switch';
const BASELINE_PNG = '/workspace/.audit/orchestrate/stencil-to-mitosis/baseline/stencil_switch_before.png';
const AFTER_PNG = '/opt/cursor/artifacts/mitosis_lit_switch_after.png';
const DIFF_PNG = '/opt/cursor/artifacts/switch_pixel_diff.png';
const CONTROL_PNG = `${HARNESS_DIR}/stencil_switch_control.png`;
...
await page.addScriptTag({ path: `${HARNESS_DIR}/lit-switch.bundle.js` });
await page.waitForFunction(() => customElements.get('lit-switch'));
const swap = await page.evaluate(() => {
  const hosts = [...document.querySelectorAll('[data-card="switch"] p-switch')];
  for (const el of hosts) {
    const lit = document.createElement('lit-switch');
...
await pauseCardAnimation(page);
await page.waitForFunction(() => {
  const hosts = [...document.querySelectorAll('[data-card="switch"] lit-switch')];
  return (
    hosts.length >= 6 &&
    hosts.every((el) => {
      ...
      return !!el.shadowRoot?.querySelector('button[role="switch"]');
    })
  );
});
swap.litRendered = await page.evaluate(
  () =>
    [...document.querySelectorAll('[data-card="switch"] lit-switch')].filter((el) =>
      el.shadowRoot?.querySelector('button[role="switch"]'),
    ).length,
);
swap.fragment = await page.evaluate(
  () => !!document.querySelector('[data-card="switch"] lit-switch')?.shadowRoot?.querySelector('my-fragment'),
);
swap.innerLit = await page.evaluate(
  () => !!document.querySelector('[data-card="switch"] lit-switch')?.shadowRoot?.querySelector('lit-spinner'),
);
