...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // output/lit/src/Link.ts
  var LitLink = class extends i4 {
    get cssText() {
      const minWidth = {
        xs: 480,
        s: 760,
      ...
      const hostRadiusFor = (h3, c4) => isTrue(h3) ? "var(--p-link-radius,var(--p-radius-full))" : "var(--p-link-radius,var(--_p-link-button-a))";
      const slottedRadiusFor = (h3, c4) => isTrue(h3) ? "var(--p-link-radius,var(--p-radius-full))" : "var(--p-link-radius," + buttonRadiusFor(c4) + ")";
      const padFor = (h3) => isTrue(h3) ? "var(--p-link-py,calc(28px * (var(--_p-link-a) - 0.64285714) + 6px)) var(--p-link-px,calc(28px * (var(--_p-link-a) - 0.64285714) + 6px))" : "var(--p-link-py,calc(28px * (var(--_p-link-a) - 0.64285714) + 6px)) var(--p-link-px,calc(33.6px * (var(--_p-link-a) - 0.64285714) + 16px))";
      const gapFor = (h3) => isTrue(h3) ? "var(--p-link-gap,0)" : "var(--p-link-gap,calc(11.2px * (var(--_p-link-a) - 0.64285714) + 4px))";
      const labelFor = (h3, baseClip) => isTrue(h3) ? "position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:" + (baseClip ? "unset !important" : "rect(0,0,0,0)") + ";white-space:nowrap" : "position:static;width:auto;height:auto;padding:0;margin:0;overflow:visible;clip:" + (baseClip ? "unset !important" : "auto") + ";white-space:normal";
      const iconMarginFor = (h3) => isTrue(h3) ? "0" : "calc(-1 * (11.2px * (var(--_p-link-a) - 0.64285714) + 4px))";
      const bg = variant === "secondary" ? "var(--p-link-bg,var(--p-color-frosted-strong))" : "var(--p-link-bg,var(--p-color-primary))";
      const fg = variant === "secondary" ? "var(--p-link-fg,var(--p-color-primary))" : "var(--p-link-fg,var(--p-color-canvas))";
      const hoverBg = variant === "secondary" ? "var(--p-link-bg,var(--p-color-frosted))" : "var(--p-link-bg,var(--p-color-contrast-high))";
      const hasIconCss = hasVisibleIcon || isTrue(hideBase) || typeof hideLabel === "object" && hideLabel !== null;
      let out = ":host{--_p-link-a:" + scaleFor(compactBase) + ";--_p-link-button-a:" + buttonRadiusFor(compactBase) + ";border-radius:" + hostRadiusFor(hideBase, compactBase) + "!important}:not(:defined,[data-ssr]){visibility:hidden}";
      if (hasSlottedAnchor) {
        out += '::slotted(a){all:unset!important}::slotted(a)::before{content:""!important;position:fixed!important;inset:0!important;border-radius:' + slottedRadiusFor(hideBase, compactBase) + "!important}::slotted(a:focus-visible)::before{outline:2px solid var(--p-color-focus)!important;outline-offset:2px!important}@media(forced-colors:active){::slotted(a:focus-visible)::before{outline-color:Highlight!important}}";
      }
      out += ".root{all:unset;display:flex;justify-content:center;width:100%;min-width:min-content;box-sizing:border-box;-webkit-backdrop-filter:var(--p-blur-frosted);backdrop-filter:var(--p-blur-frosted);font:var(--p-font-weight-normal) var(--p-typescale-sm) / var(--p-leading-normal) var(--p-font-porsche-next);border-radius:inherit;transform:translate3d(0,0,0);background-color:" + bg + ";color:" + fg + ";cursor:pointer;transition:background-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), border-color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out), color var(--p-transition-duration,var(--p-duration-sm)) var(--p-ease-in-out);padding:" + padFor(hideBase) + ";gap:" + gapFor(hideBase) + "}";
      if (!hasSlottedAnchor) {
        out += ".root:focus-visible{outline:2px solid var(--p-color-focus);outline-offset:2px}";
      }
      out += ".label{" + labelFor(hideBase, true) + "}";
      if (hasIconCss) {
        out += ".icon{font:var(--p-typescale-sm) var(--p-font-porsche-next);width:var(--p-leading-normal);height:var(--p-leading-normal);margin-inline-start:" + iconMarginFor(hideBase) + "}";
      }
      if (!hasVisibleIcon) {
        out += "p-icon{display:none}";
      }
      if (hasSlottedAnchor) {
        out += "@media(forced-colors:active){.root{forced-color-adjust:none;background:Canvas;color:LinkText;box-shadow:inset 0 0 0 2px LinkText}.root:is(button){box-shadow:inset 0 0 0 2px ButtonBorder;color:ButtonText}}";
      } else {
      ...
      out += "@media(hover:hover){.root:hover{color:" + fg + ";background-color:" + hoverBg + "}@media(forced-colors:active){.root:hover{background:Canvas}}}";
      const keys = {};
      ...
      for (const bp of Object.keys(keys)) {
        ...
        out += "@media(min-width:" + minWidth[bp] + "px){:host{--_p-link-a:" + scaleFor(c4) + ";--_p-link-button-a:" + buttonRadiusFor(c4) + ";border-radius:" + hostRadiusFor(h3, c4) + "!important}";
        if (hasSlottedAnchor) {
          out += "::slotted(a)::before{border-radius:" + slottedRadiusFor(h3, c4) + "!important}";
        }
        out += ".root{padding:" + padFor(h3) + ";gap:" + gapFor(h3) + "}.label{" + labelFor(h3, false) + "}";
        if (hasIconCss) {
    ...
    render() {
      return b2`<span class="root"><style .innerHTML="${this.cssText}"></style><p-icon class="icon" name=${this.iconName || A} source=${this.iconSrc || A} size="inherit" color="inherit" aria-hidden="true"></p-icon><span class="label"><slot></slot></span></span>`;
    }
  };
  LitLink.styles = i`
      :host {
...
`;
  __decorateClass([
    n4()
  ], LitLink.prototype, "variant", 2);
  __decorateClass([
    n4()
  ], LitLink.prototype, "href", 2);
  __decorateClass([
    n4()
  ], LitLink.prototype, "icon", 2);
  __decorateClass([
    n4()
  ], LitLink.prototype, "iconSource", 2);
  __decorateClass([
    n4()
  ], LitLink.prototype, "hideLabel", 2);
  __decorateClass([
    n4()
  ], LitLink.prototype, "compact", 2);
  LitLink = __decorateClass([
    t3("lit-link")
  ], LitLink);
})();
