...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var WORDMARK_H = "clamp(0.63rem, 0.42vw + 0.5rem, 1rem)";
  var WORDMARK_H_ROUND = `round(down, ${WORDMARK_H}, 1px)`;
  var LitWordmark = class extends i4 {
    get hostStyle() {
      ...
      return {
        "--p-wordmark-h": WORDMARK_H,
        "--p-wordmark-h-round": WORDMARK_H_ROUND
      };
    }
    connectedCallback() {
    ...
    render() {
      return b2`
          ...
          <a  .href=${this.href}  .target=${this.target || "_self"} ><svg  xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 4500 300" ><title >Porsche</title>
  ...
  LitWordmark.styles = i`
      :host {
          ...
          height: var(--p-wordmark-h) !important;
        }
        :host([hidden]) {
        ...
        @supports (height: round(down, 1px, 1px)) {
          :host {
            height: var(--p-wordmark-h-round, var(--p-wordmark-h)) !important;
          }
        }
        a {
          all: unset;
