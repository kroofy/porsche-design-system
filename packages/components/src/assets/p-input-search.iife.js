...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitInputSearch = class extends i4 {
    get hostStyle() {
      ...
      const vars = {
        ...
        "--p-it-ind-display": hasIndicator ? "inline-flex" : "none",
        "--p-it-btn-pad": hasClear ? "var(--ref-p-input-slotted-padding)" : "",
        "--p-it-btn-m": hasClear ? "var(--ref-p-input-slotted-margin)" : "",
  ...
  LitInputSearch.styles = i`
      :host {
        ...
        .wrapper > p-icon {
          display: var(--p-it-ind-display, none);
        }
        .button {
          padding: var(--p-it-btn-pad);
          margin: var(--p-it-btn-m);
        }
        :host(:not([clear])) .button {
          display: none;
        }
        .loading {
          position: absolute;
